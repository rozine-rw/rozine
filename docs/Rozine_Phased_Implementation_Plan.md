...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - [ ] Connect the existing 1B Business/Auditor surfaces to authorized Resources/commands; verify stale, denied, retry, reload and responsive browser journeys. Erastus published the Business fixture slice at `b690b3f6` in [PR #102](https://github.com/rozine-rw/rozine/pull/102); fetched for integration, not yet bound to live routes. [Live-binding follow-ups](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5808984398) cover full readable document text and persisted step transitions; lower-principal preview also remains in his UI lane. He is continuing the Auditor UI.
