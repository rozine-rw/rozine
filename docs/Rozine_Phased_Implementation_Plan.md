...
- **D-52 — `DECIDED 2026-09-07; AMENDS THE BRAND SYSTEM`:** The logo package's colours supersede the V1.0 guide values: Rozine Blue **`#0039FF`** (was `#0A5CFF`), Business **`#1D9E75`** (was `#12A150`), Auditor **`#C2661F`** (was `#DD8A00`). Ink `#0C1830`, Pulse `#08090D` and the neutrals are unchanged. This is an amendment, not a clarification, so it requires a dated V1.1 revision of the brand system, and it changes product chrome as well as the logo files — core blue is every primary button, link, focus ring and active nav state. Two derived tokens are **not yet resolved**: Deep Blue `#0A3FB0` (gradient base and pressed states, derived from the old core blue) and the Strong/Stable/Weak rating band colours, which are literally the three role colours. `#BA0D3B` is confirmed **not** part of the system: it was proposed because it appears in 18 frames, but every occurrence is a `<circle r="0.5">` export artefact present in no logo shape.
- **D-56 — `RIGHTS RECORD NOT SETTLED 2026-09-07`:** All 31 SVG and 31 matching PNG sources are received, hash-recorded and semantically mapped, and the design decisions above are made. The rights record is not closed. The answer supplied described the product's UI font and its marketing imagery rather than the logo artwork, and raises three issues that must resolve before a trademark is cut. (a) **San Francisco cannot be used for a wordmark**: Apple licenses SF for user-interface use in apps on Apple platforms and for UI mock-ups, not for creating a logo or trademark, and converting letterforms to outlines changes the file format rather than the licence. Inter, by contrast, is SIL Open Font License, which expressly permits logos — so confirming which face the wordmark was actually set in either resolves this or requires the wordmark to be redrawn. (b) **Unsplash's licence excludes using photos to create a trademark or logo**, so any Unsplash material inside the mark is a defect, though marketing imagery is unaffected. (c) **Artwork produced by prompting a generative tool** has unsettled authorship in several jurisdictions, which does not block use but bears on registrability, so the record must state which elements were generated and which were drawn. Required external legal/regulatory clearance remains a separate gate.
...
- **D-63 — `DECIDED 2026-09-07; ACCESSIBLE-NAME RULE OPEN`:** Role wordmarks pair with Rozine by default — `Rozine investor`, `Rozine business`, `Rozine auditor`. The standalone title is permitted in generic contexts where Rozine is already established. The exact boundary is still open because an accessible name cannot be conditional: a screen reader announces one string every time and it is the only identity a non-sighted user receives. Proposed for confirmation — the visible mark may drop `Rozine` inside a signed-in application; the accessible name is always the paired form; everything public-facing stays paired in both.
