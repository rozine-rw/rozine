...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - [ ] Implement Auditor accreditation/standing, conflict-safe dispatch/assignment, procedure/evidence review, factual findings, co-signature and immutable seal through the online Alpha chain; retain separate native/offline and external-evidence gates.
    ...
    - [x] Add the authorized accreditation read model and shared Resource with fresh `standing.current`/`reason`, Rwanda-calendar expiry, explicit action permissions, saved availability preference and private certificate hash metadata. Internal check references and audit notes are excluded; a certificate expiring before staff approval returns a recorded 422 validation error. Evidence: 47 focused tests / 239 assertions, 1,625 full PHP tests / 7,656 assertions at 100% application lines and zero PHPStan errors. Erastus owns the matching Home/Profile UI; live routing, command `data.next` and browser proof remain open.
