...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - [ ] Connect the existing 1B Business/Auditor surfaces to authorized Resources/commands; verify stale, denied, retry, reload and responsive browser journeys.
  - [ ] Freeze concrete protected-module boundaries in the ADR, prove negative controls and PostgreSQL races, pass full exact-commit PHP/web/CI gates, and audit every checkpoint 2 requirement before acceptance. **Local foundation verification:** the full non-TIA PHP suite passes 1,196 tests / 6,101 assertions at 100.0% application line coverage; PHPStan has zero errors, and `vp check` has zero errors with the existing 41 warnings. All 28 negative-control scenarios are caught, the clean architecture suite passes after removal, and the web foundation chain (lint, formatting, types, i18n, tests, production build and generated-route comparison) passes. This is foundation evidence; exact-commit web/hosted checks and checkpoint completion remain open.
