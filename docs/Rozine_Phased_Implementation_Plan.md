...
- [ ] Create the MVP crosswalk: 12 spine criteria, 8 Business criteria, 8 Auditor criteria, 10 Investor criteria, 8 Admin criteria, every launcher/demo requirement, all 38 screens, and every named state mapped to phase, slice, automated evidence, witnessed evidence, owner, and status. **Blocked 2026-09-09:** `Rozine MVP Spec.pdf` is not in the repository. Section 7 reproduces the 38 screens so screen coverage is recoverable, but the 46 role acceptance criteria exist only in that PDF and are not approximated here.
- [ ] Create a deferred-scope register for every PDF “Not in MVP” item and every old-plan item moved to Phases 5–8; deferral is not automatic product approval. **Partly blocked 2026-09-09:** the Phase 5–8 half is derivable from the plan; the PDF's “Not in MVP” list is not, because `Rozine MVP Spec.pdf` is absent from the repository.
...
- [x] Define environment isolation, feature flags, seeded-demo boundaries, and reset controls so demo/UAT facts cannot look live or touch real records. **Done 2026-09-10:** `docs/phase-0/environment-isolation-policy.md` defines each environment's purpose and permitted data, the seeded-demo boundaries, per-environment reset controls and the feature-flag rules. One boundary is enforced in code rather than only written: production now refuses `db:seed`, which `DB::prohibitDestructiveCommands` does not cover and which would otherwise plant a verified account with a public password beside the live waitlist. Known gaps are recorded with owners — shared deploy user, shared SSH secrets, shared mail key, one host — and one is urgent: the server runs PHP 8.4 while D-73 pins deployment to 8.5, so every deployment currently aborts. The demo environment and any flag library are defined, not built.
- [ ] Start KYC/KYB, registry, ICPAR, parsing, bank, MoMo, storage, maps, legal, and CMA tracks with named owners. **Owned 2026-09-10, not yet started:** `docs/phase-0/provider-regulatory-tracks.md` gives all ten tracks, plus four adjacent ones, a named owner, governing decision, first action and certification gate. No provider contact, contract or regulator engagement is claimed — those are acts only the owners can perform, and this item closes when each confirms first contact.
