...
- **D-51 — `OPEN; CONFLICT EVIDENCED 2026-09-06`:** Which supplied Rozine treatment is canonical: the star integrated at the `i` or the detached leading star? Confirm the standalone mark and the Investor, Business, and Auditor lockups, and define horizontal/compact, favicon/PWA, monochrome, and dark variants from a rights-cleared vector master before Phase 0 exits. The approved V1.0 brand system describes the mark as **the wing**, matching the current runtime asset `rozine-wing-white.png`, while the supplied package is star-based; that conflict is substantive and only Robert can resolve it. All 31 files remain non-semantic Figma exports (`Frame 83`-`Frame 113`), so no decision currently maps to a file.
- **D-52 — `CLOSED 2026-09-06`:** The V1.0 brand system is approved as the authoritative colour contract, so the guide's values stand: Rozine Blue `#0A5CFF`, Deep Blue `#0A3FB0`, Ink `#0C1830`, Business `#12A150`, Auditor `#DD8A00`, Pulse `#08090D`, plus the approved neutrals and rating-band colours. The colours embedded in the supplied SVG package do **not** amend them. Extraction across all 31 files found `#0039FF`, `#1D9E75` and `#C2661F` differing from the approved tokens and a crimson `#BA0D3B` — the package's most frequent fill, present in 18 frames — that is absent from the approved system entirely. Either the masters are re-exported in approved colours or a dated brand amendment adds these values; until then no production asset may be derived from the package and no application token may be changed. Recorded in `docs/phase-0/brand-approval-record.md`.
