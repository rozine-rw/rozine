...
- **D-04 — `OPTIONS PRESENTED 2026-08-29; DECISION PENDING`:** Choose (A) web/PWA-only Auditor capture, permitted only if a maximum two-day device-matrix spike proves every camera-only, encryption/key, interruption, ordered-sync, location/provenance, revocation, and purge gate; (B) a narrow thin-native secure-capture companion while all ordinary Auditor screens remain Inertia web/PWA; or (C) defer protected capture and call the result Alpha/demo rather than the governed MVP. **Recommendation:** fail closed from the two-day spike into Option B if any mandatory guarantee fails. Full native role clients still follow in Phase 5.
