...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - Fixed checkpoint 2 exit list: [Erastus’s proposal](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816417397) and [confirmed order/clarifications](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816458617). Execute S-C → S-A/S-B → S-D, with S-E in each slice; review finished slices at exact SHAs and start matching UI binding immediately. Raise listing/Investor execution stays outside C2. Co-signed audit-report persistence/publication stays inside S-D. Online isolated Alpha does not satisfy the separate D-04, C6 or operational/provider release gates.
    - [x] **S-C:** Own-conflict receipts/register, scoped offers and assignments, authoritative allowed actions/clocks and naming maps, minimal staff redispatch/close, and their web/API Resources. **Accepted slice:** Erastus approved the corrections at `eac5d4f450c32c5d750d3a97dc070567fd47e350`; all five [hosted gates](https://github.com/rozine-rw/rozine/actions/runs/36080748059) passed on that exact head. This closes S-C only; the agreed follow-ups below remain in S-A/S-B and C2 remains open.
      ...
      - [ ] Finish the remaining agreed S-C follow-ups within S-A/S-B/S-D: reduce Business/Jobs read locks and initial file queries; bind each assignment to an explicit application/revision and legitimate next-engagement trigger; use offer-time ordering with a stable cursor. Erastus's [#112 correction at `abcc06c2`](https://github.com/rozine-rw/rozine/pull/112) fixes the deadline-during-read and hidden-reconnect findings and passes all five hosted checks. Independent review reran all 259 Auditor frontend tests across 16 files; [the background-refresh slice is accepted](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5827179487) and integrated locally, with the combined branch gate still open. The ADR records current-status conflict semantics and recorded versus pre-journal validation errors.
    - [ ] **S-A:** Immutable application quote publication, review-step advancement, pinned-version signatures/submission and operation lookup.
    ...
    - [ ] **S-B:** Persisted replayable underwriting bound to verified evidence and acceptance authority, including approved exact-money/evidence-window/exposure rules.
    ...
    - [ ] **S-D:** Versioned procedure/ledger step saves, step-up, request-changes/reject/amend, actual co-signature, immutable report/seal and authorized publication/read.
      ...
      - [ ] Finish scope-bound engagement checks with the procedure delivery: shared authority reads, acceptance-versus-publication races, UUID changes to release/hash, and recovery after new terms. The [review at `79d9ef50`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5315485375) accepts the documented re-accept-then-recover policy; no replacement policy is pending.
      - [ ] Non-blocking engagement follow-up list (not an additional C2 exit gate): lighter summary reads, distinct refusal-code names, decline/conflict behavior under actual catalog corruption, legal-text control-character feedback, publish-refusal ordering/clean CLI errors for missing actors or invalid host configuration, any separately approved non-material release policy, per-offer reason metadata and self-describing previous-acceptance metadata. Escalate an item only if a concrete correctness, security or data-integrity failure affects the delivered scope.
