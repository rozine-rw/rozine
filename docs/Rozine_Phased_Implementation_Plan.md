...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - [ ] Implement Auditor accreditation/standing, conflict-safe dispatch/assignment, procedure/evidence review, factual findings, co-signature and immutable seal through the online Alpha chain; retain separate native/offline and external-evidence gates.
    ...
    - [x] Correct the first five assignment findings from [Erastus’s review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5304861890): pin PostgreSQL to UTC and use immutable explicit offer instants; retain a private 409 expiry response for a former expired recipient; reject late declines; isolate/log individual expiry failures; and limit participant read/acceptance locks to that Auditor. Acceptance still serializes the three-active-engagement cap across logins/Businesses. Late authorized source reads remain available for remediation without extending deadlines or marking reports complete. Unrelated actors receive 404 before Business-authority details. Proof: 1,848 full non-TIA PHP tests / 9,079 assertions at 100% application lines; zero PHPStan errors; 54 PostgreSQL concurrency/worker tests / 199 assertions, including expiry-versus-acceptance, unrelated-Auditor updates during protected reads, cross-Business conflict/dispatch, and the existing last-capacity-slot race. Exact-commit web/hosted gates and independent review are pending. Coded declines, own-conflict projections, scoped Jobs lists, Operations resolution and report completion remain open.
