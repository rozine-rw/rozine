...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - Fixed checkpoint 2 exit list: [Erastus’s proposal](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816417397) and [confirmed order/clarifications](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816458617). Execute S-C → S-A/S-B → S-D, with S-E in each slice; review finished slices at exact SHAs and start matching UI binding immediately. Raise listing/Investor execution stays outside C2. Co-signed audit-report persistence/publication stays inside S-D. Online isolated Alpha does not satisfy the separate D-04, C6 or operational/provider release gates.
    - [x] **S-C:** Own-conflict receipts/register, scoped offers and assignments, authoritative allowed actions/clocks and naming maps, minimal staff redispatch/close, and their web/API Resources. **Accepted slice:** Erastus approved the corrections at `eac5d4f450c32c5d750d3a97dc070567fd47e350`; all five [hosted gates](https://github.com/rozine-rw/rozine/actions/runs/36080748059) passed on that exact head. This closes S-C only; the agreed follow-ups below remain in S-A/S-B and C2 remains open.
      ...
      - [ ] Batch the agreed remaining review follow-ups into S-A/S-B: reduce Jobs read locks/background reloads; bind each assignment to an explicit application/revision and legitimate next-engagement trigger; enforce closed-row immutability with a safe rollback policy; align port/HTTP page limits; share dropped-record codes; and use offer-time ordering with a stable cursor. The ADR records the current-status conflict semantics and the distinction between recorded and pre-journal validation errors.
    ...
    - [ ] **U-A/U-B/U-C:** Erastus binds Business Apply, Auditor Jobs/file/conflict register, then procedure/seal/co-sign against the published slice contracts.
      - U-A review in progress at [#111 `1c0a820e`](https://github.com/rozine-rw/rozine/pull/111). Erastus reports passing real-prop and desktop/phone browser journeys. The [independent recovery finding](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5826480031) remains open: a state-preserving Inertia reload must not leave older form/acceptance state alongside newer server props. Combined U-A acceptance is not recorded yet. Erastus [also owns the Jobs background-refresh follow-up](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5826459830); server read/engagement/pagination follow-ups remain Aminu's work.
