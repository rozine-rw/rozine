...
- **D-52 — `DECIDED 2026-09-07; AMENDS THE BRAND SYSTEM`:** The logo package's colours supersede the V1.0 guide values: Rozine Blue **`#0039FF`** (was `#0A5CFF`), Business **`#1D9E75`** (was `#12A150`), Auditor **`#C2661F`** (was `#DD8A00`). Ink `#0C1830`, Pulse `#08090D` and the neutrals are unchanged. This is an amendment, not a clarification, so it requires a dated V1.1 revision of the brand system, and it changes product chrome as well as the logo files — core blue is every primary button, link, focus ring and active nav state. **Deep Blue** is replaced by a value derived from the new core blue; `#0027B0` is proposed, matching the ratio the guide used between `#0A5CFF` and `#0A3FB0`. Whether the **Strong/Stable/Weak rating bands** follow the amendment is the single outstanding colour question — they are literally the three role colours, and the answer restated that fact without resolving it. `#BA0D3B` is confirmed **not** part of the system: it was proposed because it appears in 18 frames, but every occurrence is a `<circle r="0.5">` export artefact present in no logo shape.
- **D-56 — `WORDMARK FONT LICENCE DEFECT CONFIRMED 2026-09-07`:** The brand owner confirmed the `rozine` wordmark is set in **San Francisco Pro**. Apple licenses the SF family for user-interface use in software on Apple platforms and for UI mock-ups, prohibits modification, and does not grant rights to use it in a logo or trademark; converting letterforms to outlines carries the restriction rather than curing it. This is an engineering reading of a licence and goes to internal Legal and, under D-39, external counsel — but it is treated as a defect until counsel says otherwise, because discovering it after registration costs far more than resetting the letterforms now. **19 of the 31 files carry letterforms and are affected** (Frames 83/84/85/86/89/90/92/93/95/96/97/98/101/102/103/104/108/109/110). **The star mark is clean** — 12 files, every crop and colourway — so favicon, app icon and avatar derivation is not blocked. Three remedies are named: reset in Inter, which is SIL Open Font License and expressly permits trademarks; commission custom letterforms, which is what most lasting wordmarks do; or license a face with an explicit logo licence. Unsplash's licence likewise excludes photos inside a mark, and generative-tool artwork has unsettled authorship bearing on registrability; both remain unrecorded.
