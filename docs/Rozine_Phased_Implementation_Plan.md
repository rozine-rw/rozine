...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - [ ] Implement resumable Business onboarding/application actions, shared web/API Resources and live Business screens using deterministic verification/provider adapters for isolated Alpha.
    ...
    - [x] Persist resumable Business/Raise drafts and append-only application versions through current Business authority. Creation/save/lookups share the journal, exact revisions reject competing edits, old retries cannot restore superseded input, and reads do not mutate drafts. The full non-TIA PHP suite passes 1,289 tests / 6,423 assertions at 100% application line coverage; PHPStan has zero errors. The 19-test / 65-assertion PostgreSQL suite includes same-Party saves across logins and concurrent edits by two mandate holders. Review-step advancement, immutable quotes, signatures and live transports remain open.
