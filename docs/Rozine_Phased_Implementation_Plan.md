...
- [ ] Complete/sign Appendix A's underwriting worksheet and disposition every vector required by the Phase 1 chain. **Review prepared 2026-09-10:** [underwriting decision review](phase-0/underwriting-decision-review.md) proposes UW-R01–UW-R07 with owners/trade-offs; [source-vector register](phase-0/underwriting-vector-review.json) preserves all 48 dispositions and [synthetic examples](phase-0/policy-review-fixtures.json) provide partial arithmetic evidence only. No signatures, source-status changes, baselining or activation are asserted.
- [ ] Close D-26/D-27 and freeze the secondary-market contract before the Holding, Order, reservation, fee-posting, record-date, halt, and settlement schemas are finalized; mandatory secondary scope may not be traded away for schedule. **Review prepared 2026-09-10:** [secondary contract review](phase-0/secondary-contract-review.md) expands both Option A recommendations into ten candidate rules, lifecycle/atomicity/cutoff contracts and twelve required implementation scenarios. The RWF 5,000 minimum versus remaining-cashflow cap conflict is explicit; owner/external decisions and schema freeze remain pending.
...
- [ ] Define environment isolation, feature flags, seeded-demo boundaries, and reset controls so demo/UAT facts cannot look live or touch real records. **Backend groundwork locally verified 2026-09-10:** separate demo/UAT database identities and resource namespaces, opt-in demo seed/reset flags, production/UAT command prohibitions, non-live outbound restrictions and target-specific deployment configuration checks are implemented with negative tests. See Phase 0J in [kickoff evidence](phase-0/kickoff-evidence.md). This remains open pending host/DB-role/network proof, synthetic fixture/reset acceptance, visible non-live labelling and non-author/exact-SHA review; no hosted isolation or complete demo is claimed.
- [ ] Start KYC/KYB, registry, ICPAR, parsing, bank, MoMo, storage, maps, legal, and CMA tracks with named owners. **Preparation complete 2026-09-10:** [provider dependency register](phase-0/provider-dependency-register.md) covers 13 tracks, all IR-1–IR-8, proposed named leads/review dates, required inputs, fake-case specifications and scope-specific external gates. All records are `PREPARED_NOT_STARTED`; actual owner acknowledgement/kickoff, vendor selection, sandbox and certification evidence remain absent from this register and are not claimed.
