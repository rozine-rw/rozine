...
- [ ] Integrate checkpoint 3: wallet, Investor purchase, ledger, primary settlement, and Admin disbursement.
  - [ ] S3-A: final Business acceptance/exposure, staff release, campaign publication/progress/cancellation. Server work started from `dev` `f2cc72ce`; Erastus retains UI/browser ownership ([coordination](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5849001511)).
    - [x] Implement the AC-02 final-signature reservation boundary: one full-principal reservation in the submission transaction, immutable parent bindings and encrypted evidence, exact accepted-exposure arithmetic/deduplication, quote freshness, replay safety, and no inferred reservation for historical C2 submissions. Local focused proof: 90 tests / 1,206 assertions, including the complete architecture suite, PostgreSQL concurrent signatures/retries and web/API receipt/reload parity; the separate six exact-arithmetic tests also pass. Full gates and non-author review remain pending; this does not complete S3-A.
