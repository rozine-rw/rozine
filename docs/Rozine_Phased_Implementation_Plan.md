...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - [ ] Retain original statements and hashes, parse and reconcile all declared rails, append corrections and immutable verified monthly snapshots; expose parsing feedback and actionable denials.
    - [x] Retain exact encrypted PDF/CSV originals with server hashes and immutable extraction lineage through current Business authority, revision checks and the shared journal. Read-only manifests expose metadata, and authorized original reads verify integrity. PDF text and structured CSV extraction return explicit review feedback without fabricated financial values; every receipt remains `INGESTED_NOT_AUDIT_APPROVED`. Full PHP evidence: 1,339 non-TIA tests / 6,618 assertions at 100% application line coverage, zero PHPStan errors, and three evidence-boundary bypass probes caught with clean teardown. Bank-format mapping, classification, all-rail reconciliation, amendments, verified monthly snapshots and live uploads remain open.
    - [x] Implement pure exact-RWF reconciliation of complete declared rail/month transcriptions: source membership, opening/closing continuity, duplicate conflicts, separate operating/financing/transfer/draw/debt classes, same-month returned draws and specifically authorized one-off exclusions. The output is always unverified. Evidence: 70 focused tests / 156 assertions; full non-TIA PHP suite 1,409 tests / 6,776 assertions at 100% application line coverage; zero PHPStan errors. Persisted transcription/amendment history, authoritative source verification, Auditor approval, immutable verified snapshots and live transport remain open.
