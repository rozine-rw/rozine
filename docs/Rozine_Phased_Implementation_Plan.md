...
**First implementation slice:** `feat/phase-1-identity-foundation`, based on the verified `origin/dev` revision above. Fortify registration delegates atomic account/Party creation to the shared identity action and persistence port. New Parties are unverified and receive no memberships. `dashboard` and `api.v1.identity.show` expose the same `identity-v1` Resource through `GetIdentityContext`; Investor/Business coexistence and Auditor conflicts are evaluated from current Party memberships. The response is a read-only membership-availability contract with no transactional `allowed_actions`. Existing accounts are not silently backfilled or verified. Identity resolution across separately registered Parties, membership provisioning, active-role switching, MFA/consent, entity mandates, privileged staff accounts and the launcher UI remain checkpoint 1 work. No whole checkpoint, Phase 0 exit, Alpha acceptance or deployment is claimed.

**Local kickoff verification (PostgreSQL and review fixes, 2026-09-23):** 730 full non-TIA PostgreSQL PHP tests / 3,840 assertions pass with 100.0% application line coverage. The local application uses PostgreSQL 18.4 with all three existing Pulse signups preserved; the original SQLite file remains intact. Tests use separate non-superuser owners for `rozine_test` and disposable `rozine_demo`, denied access to the application database. Both PHPUnit configurations retain their test database and role even when conflicting application settings are inherited. Fresh password-authenticated server provisioning, schema inventory comparison, identity migration rollback/reapplication with account preservation, the coverage negative control and a Pulse HTTP 200 check pass. Both P2 review fixes are covered: rollback explicitly removes the Party index before its column, and registration sends a usable email-verification link while retaining the separate identity-verification gate. Pint, PHPStan and all 329 web tests pass; `vp check` reports zero errors and the existing 32 warnings. Prior TypeScript/build and source/risk-manifest evidence remain separate from these fixes. Changes remain uncommitted: exact-commit client provenance/changed-branch admission, hosted checks (configured for PostgreSQL 17) and checkpoint acceptance remain outstanding.
...
- [ ] Implement the Fortify identity/Party model, role membership, active-role authorization, MFA/consent rules, and server contracts for the launcher; reassess the two approved Fortify exceptions without duplicating them for mobile. **IN PROGRESS — registration and read-only role availability.**
  ...
  - [ ] Implement controlled identity resolution across separately registered Parties and membership provisioning/lifecycle actions.
  - [ ] Implement persistent active-role selection and server-side authorization for role-scoped actions.
  - [ ] Implement the governed MFA, consent, recovery, entity-mandate and privileged-staff authority rules and complete the server launcher contract.
...
- [ ] Add PHP unit, feature, Resource/API parity, property, denial, idempotency, PostgreSQL concurrency, and architecture tests with each slice; maintain 100% PHP line coverage and zero-error Pest-aware PHPStan/Larastan. **IN PROGRESS — local identity evidence.**
  ...
  - [x] Run the full local non-TIA PostgreSQL PHP coverage suite and PHPStan/Larastan for the initial identity slice, database switch and two P2 review fixes: 730 tests, 3,840 assertions, 100.0% application line coverage and zero static-analysis errors, as recorded in Local kickoff verification above.
