...
- [ ] Implement the Fortify identity/Party model, role membership, active-role authorization, MFA/consent rules, and server contracts for the launcher; reassess the two approved Fortify exceptions without duplicating them for mobile. **IN PROGRESS — controlled identity access implemented; remaining checkpoint 1 rules open.**
  ...
  - [ ] Publish authorized role-home adapters and the separate staff-entry contract. The [issue #96 handoff](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5800301779) proposes `investor.home`, `business.home`, `auditor.home` and `admin.home` at the corresponding root paths; these are contract reservations, not registered routes. Marketplace homes must enforce the selected role; the identity-operator bootstrap does not imply Admin access. The proposed versioned `staff_access` projection and scoped staff policy require joint contract freeze and implementation.
  - [ ] Freeze and implement per-account/per-role return-position contracts with Phase 1B. The [issue #96 proposal](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5800301779) uses allowlisted named-route bookmarks with validated parameters/query and minimal presentation state, renewed authorization before restoration and a role-home fallback. It excludes saved sensitive page props and arbitrary redirect URLs. Return bookmarks are not implemented or included in `identity-v2`.
...
**Status, 2026-09-23:** `IN PROGRESS` on `feat/phase-1b-design-foundation`; the source/fixture snapshot inspected for [issue #96](https://github.com/rozine-rw/rozine/issues/96) is [`613e3e4`](https://github.com/rozine-rw/rozine/commit/613e3e4a673f4a1e20887b0d99a183cc690308f8). It contains the launcher/design foundation and Business sign-in, home, application, listing, onboarding and note-progress surfaces. The launcher types and three identity fixtures still target `identity-v1`; [the published handoff](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5800301779) specifies `identity-v2`, the JSON selection command, allowed-action handling and required denied/stale/MFA states. This records source/contract coordination, not full UI review, rendered acceptance or completion of the checklist below. Page fixtures remain proposals until reconciled with the governed server contracts.
...
- [ ] Integrate checkpoint 1: Party, authorization, active role, and launcher. **IN PROGRESS — contract handoff published; integration remains open.**
  ...
  - [ ] Confirm the proposed role-home, return-position and staff-access contracts together; implement the remaining 1A adapters and update the 1B launcher/types/fixtures against `identity-v2`.
  - [ ] Promote 1A to `dev` through its reviewed, passing PR, then rebase/integrate 1B. [PR #97](https://github.com/rozine-rw/rozine/pull/97) is open against `dev`, with Erastus requested for the final engineering review; hosted checks and review remain separate from the completed local evidence. Reconcile the shared identity type/export, `routes/web.php` and client source/risk manifests; preserve the server-backed dashboard and identity routes, regenerate/validate combined manifests, and run the checkpoint's exact-commit tests and browser journeys.
