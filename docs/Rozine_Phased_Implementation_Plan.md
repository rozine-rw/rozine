...
- [ ] Complete the existing `tests/Architecture/ArchitectureTest.php` rule catalog with module-specific architecture files as needed and cover the Section 4 dependency rules, naming/inheritance rules, strict types, prohibited debug calls, and all Domain/Application/HTTP/Resource/Integration boundaries defined in Section 11. **Substantially done 2026-09-06:** every rule is now stated over namespaces rather than over named classes, and the catalog covers dependency direction, naming and inheritance, prohibited debug and termination helpers, `config()` over `env()`, Domain purity including clock and randomness, Application-through-contracts, Resource isolation from persistence, and adapters reachable only through their container binding. Negative controls prove three of these fail on a planted violation. **Two gaps remain:** no strict-typing convention has been approved, so no rule enforces one — 47 of 48 first-party files omit `declare(strict_types=1)`; and the protected ledger, settlement, seal and immutable-evidence rules cannot be written until those namespaces exist, which happens with the module that introduces them.
