...
- [ ] Complete the existing `tests/Architecture/ArchitectureTest.php` rule catalog with module-specific architecture files as needed and cover the Section 4 dependency rules, naming/inheritance rules, strict types, prohibited debug calls, and all Domain/Application/HTTP/Resource/Integration boundaries defined in Section 11. The 2026-08-29 working tree closes the specific `PulseController` legacy exception with tested Domain/Application/Infrastructure/HTTP boundaries; the broader catalog, negative controls, and clean exact-SHA evidence remain open.
...
- [x] Prove the approved runtime contract through the hosted PHP 8.5 gate, pin PHP 8.5 in deployment evidence, and add PostgreSQL-backed locking/concurrency tests; SQLite may remain a fast unit/feature lane but cannot certify races. **Done 2026-09-07:** the deployment script resolves 8.5 explicitly and aborts otherwise, and the PostgreSQL lane runs the suite plus a Concurrency suite that exists only there.
...
- [ ] All 31 current SVG sources and 31 matching PNG references are accounted for; one primary lockup/mark direction and the role naming contract are approved; rights-cleared vector/transparent masters exist; no unapproved source is treated as a production master.
...
- **D-51 — `MARK CONFIRMED AS THE STAR 2026-09-07; PRIMARY LOCKUP OPEN`:** Inspection of all 31 renders confirms the mark is a five-pointed star with a swept lower-left tail. The approved V1.0 guide's section 07 calls the mark **the wing** and the live runtime asset is still `rozine-wing-white.png`; both describe the previous mark, and the star supersedes it. The package also has no duplicate problem: grouping by path geometry resolves 31 files into 8 artworks in their colourways, recorded in `docs/phase-0/brand-asset-manifest.json`. What remains open is the primary lockup — the detached leading star (Frames 83/84/89/92/95/96/101/102) or the star integrated at the `i` (Frames 85/90/93/97/98/103/104). The role wordmarks all integrate the star at the `i`, which argues for the integrated treatment as the system, but only Robert can choose. If the integrated lockup is chosen, its only blue-on-white export is Frame 86, which is defective — exported unflattened at 19 paths with a stray rectangle on the `e` — and must be re-exported.
- **D-52 — `CLOSED 2026-09-06; ONE AMENDMENT DECLINED 2026-09-07`:** The V1.0 brand system is approved as the authoritative colour contract, so the guide's values stand: Rozine Blue `#0A5CFF`, Deep Blue `#0A3FB0`, Ink `#0C1830`, Business `#12A150`, Auditor `#DD8A00`, Pulse `#08090D`, plus the approved neutrals and rating-band colours. The package's `#0039FF`, `#1D9E75` and `#C2661F` differ from the approved tokens; either the masters are re-exported in approved colours or a dated amendment adopts the package values, and that must settle before a master is cut. **`#BA0D3B` was proposed for addition to the system on the grounds that it is used in the logos; it is not, and the amendment has not been made.** All 48 occurrences across 18 frames are `<circle r="0.5">` on canvases 2796-7545 units wide — about one fifteen-thousandth of the width, invisible at any rendered size. Frame 83 renders solid blue while containing three of them. It is Figma export residue, and adopting it would enshrine an artefact as a brand colour and propagate it into design tokens. A crimson may still be added on its own merits.
- **D-56 — `SVG/PNG RECEIVED AND HASHED 2026-08-29; BRAND APPROVAL PENDING`:** The user confirmed Robert supplied 31 SVG logo sources and 31 matching PNG references. All SVGs parse as XML and have immutable SHA-256 records; all PNGs are valid RGBA with alpha, have immutable hashes, and match their SVG basenames and dimensions one-to-one. The source `PNGs.zip` archive is recorded at SHA-256 `de04dac504360a6e05866707e55677b18253f7bc4b1d7293303aca0cbd1675bc`. PNG receipt is resolved, but receipt and technical validation do not approve production use. Phase 0 remains blocked until competing/duplicate files are semantically mapped, canonical vector masters are selected, exact colors and vector/font/outline provenance are established, usage rights, accessibility, and required variants/surface rules are approved, and Robert gives explicit Brand approval. PNG cannot substitute for an approved vector master. D-51, D-52, D-57, and D-63 remain open.
