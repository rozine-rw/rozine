...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - Fixed checkpoint 2 exit list: [Erastus’s proposal](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816417397) and [confirmed order/clarifications](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816458617). Execute S-C → S-A/S-B → S-D, with S-E in each slice; review finished slices at exact SHAs and start matching UI binding immediately. Raise listing/Investor execution stays outside C2. Co-signed audit-report persistence/publication stays inside S-D. Online isolated Alpha does not satisfy the separate D-04, C6 or operational/provider release gates.
    ...
    - [ ] **S-D:** Versioned procedure/ledger step saves, step-up, request-changes/reject/amend, actual co-signature, immutable report/seal and authorized publication/read.
      ...
      - [x] Implement the [agreed engagement entry summary](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5828936059) and corrections 1–2 from [Erastus's `d24a6e95` review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5315181399). Auditor Home/Profile/Jobs/file expose only current/required/unavailable status and the matching web/API agreement link; other role homes return null. A valid retained release for another procedure becomes unavailable while actual corruption remains an integrity failure. Existing offers remain readable, decline/conflict remain available, and expiry/Operations continue. The guarded `auditor:engagement-terms` command publishes or withdraws full originals from bounded local JSON through current compliance authority, with explicit revision/UUID and nonzero refusal exits; UAT/production can accept non-synthetic input without enabling synthetic seeding. Local proof: 2,525 full non-TIA PHP tests / 14,312 assertions at 100% application lines; 85 final affected tests / 701 assertions; zero PHPStan errors; clean Pint and matching inventory. All five hosted checks pass on the preceding `d24a6e95` head. Correction-head web/hosted checks and Erastus's re-review remain open. Erastus owns the agreement Inertia page/controller split, UI types/banners and browser proof; no UI completion, real terms publication or promotion is claimed.
      - [ ] Complete engagement review items 3–4 in S-D: database-visible acceptance/Party pins on reports and source verifications, current-active-release enforcement on acceptance/report insertion, immutable retention, and fabricated/stale-pin regression coverage.
      - [ ] Complete engagement review items 5–7 and remaining tests/nits in S-D: shared authority read locks, deliberate receipt-access and refusal-code policy, acceptance-versus-publication race, UUID changes to release/hash, recovery after new terms, independence of decline/conflict from corrupt catalog data, legal-text control-character feedback, consistent publish-authorization ordering and any approved non-material release policy. The ADR records the current explicit refusal matrix and immediate re-acceptance behavior; per-offer refusal reasons and previous-acceptance UI metadata remain integration follow-ups.
