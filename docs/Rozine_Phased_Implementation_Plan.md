...
- [x] Integrate checkpoint 1: Party, authorization, active role, and launcher. **Merged 2026-09-24:** [PR #98](https://github.com/rozine-rw/rozine/pull/98) is in `dev` at `c62aaf0f67d523f44addbda6b4d46fc0d8f92978` after all five hosted gates passed on `5f6bff8`. Aminu explicitly instructed the merge; no non-author review is represented as performed.
  ...
  - [x] Pass [PR #98](https://github.com/rozine-rw/rozine/pull/98)'s exact-commit hosted gates and promote into `dev` under Aminu's subsequent explicit merge instruction. All five checks passed; non-author review was not performed. This records the authorization for #98 separately from #97.
- [x] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **ACCEPTED AND MERGED, 2026-09-25:** Aminu authorized the merge after all implementation, review and validation gates passed at `c50e035ef877c4e5c6b51e0d32422c8d9141ef8e`. [PR #101](https://github.com/rozine-rw/rozine/pull/101) merged into `dev` as [`bb0fa320`](https://github.com/rozine-rw/rozine/commit/bb0fa3204bf672d66043581fe76840fb7dc30cb9), preserving the integrated source history. This closes checkpoint 2 for isolated Alpha; later checkpoints and deployment/provider/device/legal release gates remain separate.
  - **Final candidate evidence, 2026-09-25:** all five [hosted gates](https://github.com/rozine-rw/rozine/actions/runs/36160878125) pass on the exact candidate. Hosted PHP quality passes 3,008 full non-TIA tests / 17,810 assertions at 100% application lines with zero PHPStan errors and matching generated inventory; the independent hosted PostgreSQL lane passes the same 3,008 tests / 17,811 assertions. The exact-commit web gate passes 1,212 tests across 90 files, 100% statements/branches/functions/lines and D-67 across 334 authored files, including lint/format/types/i18n/build/generated checks. All 15 committed real browser journeys pass together on the candidate (116 assertions, zero skips), and current phone/desktop captures were inspected. [Erastus’s independent U-D handoff](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5835905122) passes all eight edge/retry journeys twice with zero skips, plus the publication happy path, from a clean detached checkout and fresh build. His [final confirmation](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5836399026) accepts every C2 gate on his side. This final record supersedes the pending-validation statements in the historical delivery evidence below. J-1/J-2/J-3 are complete; the agreed follow-ups below remain explicitly unimplemented.
  - Coordination: check Erastus's latest branch, PRs and issue comments before each slice. [Ownership and overlap handoff](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5807775265) identified duplicate launcher work; [Erastus reconciled it at `998eb5c`](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5807829460), preserving current `dev` staff/bookmark/access-refresh behavior. He confirmed no other unmerged checkpoint 2 work and ownership of Business/Auditor UI and web/browser tests; Aminu owns server/domain/persistence/Resources. Erastus [confirmed business-application-v1](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5807962622) and its [operation/signature details](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5808085096); his UI branch is `feat/phase-1b-business-application-v1`. Erastus [confirmed auditor-filing-v1](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5808172589), followed by [conflict visibility and factual-reason details](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5808266398). [Issue #99](https://github.com/rozine-rw/rozine/issues/99) tracks copy/design decisions; proposed fixture values do not supersede the current engineering contract.
  - Fixed checkpoint 2 exit list: [Erastus’s proposal](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816417397) and [confirmed order/clarifications](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816458617). Execute S-C → S-A/S-B → S-D, with S-E in each slice; review finished slices at exact SHAs and start matching UI binding immediately. Raise listing/Investor execution stays outside C2. Co-signed audit-report persistence/publication stays inside S-D. Online isolated Alpha does not satisfy the separate D-04, C6 or operational/provider release gates.
    - [x] **S-C:** Own-conflict receipts/register, scoped offers and assignments, authoritative allowed actions/clocks and naming maps, minimal staff redispatch/close, and their web/API Resources. **Accepted slice:** Erastus approved the corrections at `eac5d4f450c32c5d750d3a97dc070567fd47e350`; all five [hosted gates](https://github.com/rozine-rw/rozine/actions/runs/36080748059) passed on that exact head. This closes S-C only; the agreed follow-ups below remain in S-A/S-B and C2 remains open.
      - [x] Add private own-conflict receipt and bounded register ports through current Auditor identity/MFA. Reassignment or a lapsed Business mandate does not erase the declarant’s own history; revoked membership and stale identity context still deny it. Journal receipts retain only a conflict ID and coarse recorded outcome; explanations stay encrypted. Local proof: 65 affected feature tests / 314 assertions; 1,927 complete non-TIA PHP tests / 9,348 assertions at 100% application lines; PHPStan zero; 61 PostgreSQL concurrency/worker tests / 220 assertions, including identity withdrawal during own-conflict reads and a replacement-Auditor attestation racing the former Auditor’s correction. The remainder of S-C and its transport/UI acceptance remain open.
      - [x] Add bounded, current-authority Jobs/case-summary ports and shared web/API Resources, per-record actions and original clocks, recorded command/lookup recovery, private conflict pages, and scoped Operations redispatch/close. Missing draft/evaluation/procedure facts remain unavailable. Redispatch preserves the deadline/tried set/attempt cap; closure is separate from report completion. Local server proof: 1,971 complete non-TIA PHP tests / 9,744 assertions at 100% application lines; PHPStan zero; 68 PostgreSQL concurrency/worker tests / 246 assertions, including case reads against authority/draft changes and duplicate/competing Operations resolutions. The [U-B projection contract](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5817278371) is [confirmed by Erastus](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5817291027). Matching UI integration, rendered/browser proof, hosted gates and the finished-slice review remain open; the S-C parent is not closed by these server tests.
      - [x] Integrate Erastus's live U-B source from [PR #109](https://github.com/rozine-rw/rozine/pull/109) at `ddaf368fc159f3b726465962d392adc40c916ae7`, preserving its history. Jobs/file/conflict pages now consume the real nullable facts, per-record actions, original clocks and private receipts; conflict commands send the server's `reason` field. Jobs links to the declarant's register, and Profile links to Jobs. Local proof: 242 affected/preview tests / 2,013 assertions; 1,982 complete non-TIA PHP tests / 9,945 assertions at 100% application lines; zero PHPStan errors; and the committed checkpoint 1 browser regression (desktop/phone, return position, stale access, MFA and staff entry). The later browser and focus/reconnect integration is recorded below. Repository billing no longer blocks CI: all five hosted gates passed at PR #101's `d96307f2` and PR #109's `8b27b1a6`. The combined candidate's hosted gates and finished-slice review remain separate requirements.
      - [x] Render the access-denied page after identity withdrawal on all four new Auditor page routes. Four regressions first reproduced raw JSON where Inertia expected a page; cold browser navigation and Inertia reloads now receive the denied component without case props, while `/api/v1` retains its JSON contract. The affected profile/Jobs/UI-contract suite passes 38 tests / 884 assertions. Erastus's matching focus/reconnect behavior is integrated from `8b27b1a6`, following the [integration review](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5818189655).
      - [x] Integrate and run Erastus's committed Jobs browser journey from `8b27b1a6` with the checkpoint 1 regression against isolated `rozine_test`: both tests pass together, with desktop and 390×844 phone captures inspected. The journey verifies MFA login, live offers/countdowns, authoritative late-acceptance rejection, decline, accept, own conflict receipt/register, withdrawn file denial and membership withdrawal on focus. The expired fixture now preserves consistent original clocks; the harness asserts HTTP 409 / `ASSIGNMENT_ACCEPTANCE_EXPIRED`, captures failures, drains server logs and owns server cleanup. No listeners remain after success or failure. Combined proof at `6b0014189d155a0e26b434c713c833fad829ee7a`: 1,987 complete non-TIA PHP tests / 10,040 assertions at 100% application lines; zero PHPStan errors; Pint clean; generated inventory matches; 68 PostgreSQL concurrency/worker tests / 246 assertions; and 940 web tests with all four coverage metrics at 100% across 316 authored files. All five [hosted gates pass](https://github.com/rozine-rw/rozine/actions/runs/36076848355). [Erastus's finished-slice review is requested at that exact commit](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5824476093) and remains open; this does not close S-C or C2.
      - [x] Correct items 1–4 from [Erastus's S-C review of `6b001418`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5311847230), following the [recorded decisions](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5824813575): refuse unqualified requests after engagement closure with the original assignment ID in the immutable refusal; check staff permission before case/Business lookup without reversing lock order, then recheck under the protected transaction; keep API navigation and command recovery on API routes; and prefer submitted application facts over a newer draft. Staff resolution now requires both read/manage token abilities, withdrawn staff pages render the denied component, and browser cleanup preserves the original failure. Regression evidence covers real/missing case indistinguishability, actual bearer-only link following, a fully dropped page retaining its continuation, close-versus-request, redispatch-versus-capacity/expiry and revocation after preflight. Local proof: 120 affected feature tests / 930 assertions, four new PostgreSQL races / 22 assertions, zero PHPStan errors and 2,031 full non-TIA PHP tests / 10,354 assertions at 100% application lines. Exact-commit browser/web/hosted gates and Erastus's follow-up review remain open.
      - [x] Verify the corrected implementation at `cc61b3ab732fb6672ba797d85f6af0dab51b6a65`: all 78 PostgreSQL concurrency/worker tests / 296 assertions pass; the exact-commit web gate passes 940 tests over 81 files, all four coverage metrics at 100%, D-67 across 316 authored files, types/lint/format/i18n/build and generated-route checks; and both committed browser journeys pass together against isolated `rozine_test`. Desktop file and 390×844 phone Jobs captures were inspected, and both owned server ports are free afterward. The generated inventory includes the new source-authority guard and pure underwriting composer and matches after the browser run. Follow-up review and hosted checks of the published candidate are still required.
      - [x] Obtain [Erastus's explicit S-C approval at `eac5d4f450c32c5d750d3a97dc070567fd47e350`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5312125442). He verified all four requested corrections, the staff ability/renderer changes and browser cleanup. This review explicitly excludes the source-authority and underwriting-composer commits `6f8120c` and `09e3671`, which remain for S-A/S-B review. All five hosted checks passed at that head on 2026-09-25. Whole-checkpoint acceptance and the S-A/S-B review remain open.
      - [x] Enforce closed-assignment immutability in PostgreSQL, refuse protection rollback after closure, align Jobs/conflict port and HTTP limits at 50, and share the dropped-record denial codes. Regression coverage proves direct mutation/deletion denial, preserved receipts and data after refused rollback, safe unused rollback/reapplication, 50/51/100 page boundaries, a 51-record continuation and parity between list/operation denial handling. Included in the 2,295-test full local gate below; finished-slice follow-up review remains open.
      - Read/assignment follow-ups are retained in the non-blocking list below under the [joint C2 exit assessment](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5834433775). The accepted #112 refresh correction is integrated; this classification does not claim the remaining follow-ups are implemented.
    - [x] **S-A:** Immutable application quote publication, review-step advancement, pinned-version signatures/submission and operation lookup. Core implementation is independently accepted at `1dd41ad4`, with U-A accepted at `45df0d2c`; the combined checkpoint branch through `798ac78c` passes all five hosted gates. The evidence and corrections are recorded below. Whole-checkpoint browser and final-candidate acceptance remain J-1/J-2/J-3 gates.
      - [x] Add a guarded Business/source-verification callback for evaluation persistence. It holds the effective Business mandate and the source Auditor's identity in one ordered Party lock set, then holds professional standing through the effect. Missing or withdrawn source evidence stays explicit so evaluation can record a refusal; an author changing before the locks are acquired fails before the callback. The lock set grants no extra mandate permissions. Local proof: 102 affected feature tests / 365 assertions, six PostgreSQL races / 27 assertions, zero PHPStan errors and 2,005 full non-TIA PHP tests / 10,104 assertions at 100% application lines. The suspension race first reproduced an unlocked standing change and passes with the profile lock. Immutable quote calculation/persistence and signing are still open.
      - [x] Compose the approved cash-flow evidence, scorecard, pricing and capacity engines into a private, deterministic replay record (`ApplicationUnderwriting`). It retains original-request rejection, maximum versus accepted principal, exact ratios/schedules, repeat-window rules, audited exposure and explicit unavailable-history/restriction refusals. The composer grants no borrowing authority and does not expose private components through a Resource. Its 14 new unit tests are included in the 2,031-test / 100% application-line gate above. Publication, canonical platform history/exposure sources and quote/signature persistence remain open.
      - [x] Persist explicitly sourced, versioned isolated Alpha credit facts and immutable application quotes. Missing/withdrawn history cannot become a first-time or zero-exposure assumption. Source revisions, exact draft, effective mandate, actor/time, policy/calculator, quotation calendar and the deterministic private result are bound into encrypted immutable quote history. The public projection contains only the agreed offer/refusal fields, retains the maximum offer when a lower amount is selected, and advances an unchanged ready draft to Review. Draft/source/mandate/calendar changes invalidate current use; historical UUID receipts remain immutable. Local proof: 2,118 full non-TIA PHP tests / 10,730 assertions at 100% application lines; zero PHPStan errors; 940 web regressions; Vite+ checks pass with 40 existing warnings; all five Business-model bypass probes rejected with a green teardown. All 85 PostgreSQL concurrency/worker tests pass (320 assertions), including duplicate/competing quote publication and source withdrawal during the actual write; the generated schema/module inventory matches. These facts are explicitly synthetic Alpha inputs; the live servicing/ledger adapter belongs to C3/C4. Pinned legal acceptance, signatures, submission, live web/API snapshots and S-A/S-B non-author review remain open.
      - [x] Record pinned legal acceptance for every actual required Party and atomically submit only after all signatures exist for the identical quote, mandate and consent release. Immutable encrypted signatures/submissions bind document bodies/hashes, policy, actor/time and the explicit CFG-01 zero-fee waiver. A second login cannot substitute for another Party; superseded quote/legal versions do not reuse signatures. Final-write failure rolls back the signature, submission and journal together; retries/lookup retain their original receipts. Submitted snapshots remain historical and do not issue a Note. Local proof: 2,193 full non-TIA PHP tests / 10,979 assertions at 100% application lines; zero PHPStan errors; Pint clean; 940 web tests; Vite+ checks pass with 40 existing warnings. The 78 focused/concurrency tests / 246 assertions include seven new PostgreSQL acceptance cases proving duplicate/competing submissions and source/authority locks through the actual final write. All seven protected Business-model bypass probes fail as intended with a green teardown, and the generated schema/module inventory matches. The [early route/Resource handoff](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5825729013) unblocked [Erastus's U-A work](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5825736628); omitted-step autosaves preserve the saved pointer, and sole traders keep a null RDB company code. Live transports, browser proof and non-author S-A/S-B review remain open.
      - [x] Expose shared Apply page/create/save/evaluate/submit/lookup routes over web and `/api/v1`, with explicit audience Resources, current Business authority and scoped API abilities. Evaluate checks the displayed target/term/evidence version against locked server facts. Back only changes the rendered view; omitted-step autosave preserves the stored pointer. Historical receipts retain their data with fresh clocks/current capabilities, and submitted evidence stays pinned. Missing legal content/financial history remains unavailable; actual rolling periods and partial calendar years are reported without CRB or OCR claims. Local proof: 2,216 full non-TIA PHP tests / 11,260 assertions at 100% application lines, including the real bearer-token draft → quote → lower selection → Review → submit → lookup journey; zero PHPStan errors, Pint clean, 940 web regressions and Vite+ checks with 40 existing warnings. [Erastus’s U-A PR #111](https://github.com/rozine-rw/rozine/pull/111) owns the UI/locale/recovery bindings. The live Business Home entry list, combined real-prop/browser proof and non-author S-A/S-B acceptance remain open.
      - [x] Add a real, bounded Business application entry Resource to the existing role landing page and `/api/v1/business`. It discovers only the current Party’s effective related mandates, reauthorizes every projected record, rechecks final identity, and preserves continuation when an entry loses authority. View-only members receive read links without create; API read-only tokens receive no mutation capabilities. Entries expose current draft/submitted metadata and supplied create/resume/lookup links, with no wallet/capital stand-ins. Local proof: 2,232 full non-TIA PHP tests / 11,412 assertions at 100% application lines; zero PHPStan errors; Pint clean; production build passes. Sixteen Home tests cover current-mandate isolation, expiry/revocation, view-only access, cursor bounds, unexpected failures, membership withdrawal after projection and exclusion of an entity whose verification changed while other authorized entries remain available. [Erastus accepted the contract](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5826052233) and owns its U-A presentation on #111. Combined live UI/browser acceptance and S-A/S-B review remain open.
      - [x] Correct [Erastus's S-A/S-B review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5313241245) items 1–4 and 6: serialize the one-unresolved-submission C2 rule under the Business lock; revalidate descriptive Review fields when signing after omitted-step autosave/requote; advertise Submit only at a valid Review step; return recorded `APPLICATION_STEP_INVALID` 422 and `APPLICATION_PENDING_REVIEW` 409; expose the authorized pending-application link and withhold impossible Home/Apply actions; use Kigali calendar months while preserving UTC instants. PostgreSQL protects submitted application rows, coherent status/step/pointers, one submission per application and one unresolved submission per Business; used protection refuses rollback. Local proof: 2,295 full non-TIA PHP tests / 12,171 assertions at 100% application lines; zero PHPStan errors; Pint clean. New races cover create versus final submit, autosave versus submit, and requote versus a pending signature. The [UI code/link contract](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5826704712) is agreed; hosted checks and Erastus's correction review remain separate gates. This does not implement C3 commitment accounting or close S-A/S-B.
      - [x] Obtain [Erastus's explicit S-A/S-B core approval at `1dd41ad4`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5313525521). His real-PostgreSQL repro/bypass review confirms items 1–4 and 6, including legacy drafts, blank Review fields, direct Submit, competing commands and database terminal guards. This is core-slice approval only: read locks, engagement binding, offer-time cursors, freshness and batched nits remain separate, and C2 is not accepted. The next agreed server slice is S-D; later post-submission lifecycle migrations must extend the pending-state query/index and update the terminal trigger/CHECK together.
    - [x] **S-B:** Persisted replayable underwriting bound to verified evidence and acceptance authority, including approved exact-money/evidence-window/exposure rules. The core and D-14 freshness slices have independent acceptance at `1dd41ad4` and `4af5f27d`; the persisted repeat-borrower, evidence, exposure and exact-money proof is recorded below. Live servicing inputs remain outside isolated C2.
      - [x] Exercise the actual persisted evidence → credit facts → quote → signature/submission path for a qualified 12-month repeat borrower, with the exact RWF 5,000 offer grid and repayment schedule. [Persistence regressions](../tests/Feature/BusinessUnderwritingPersistenceTest.php) also prove refusal for incomplete 36/12-month windows and missing repeat prerequisites, deduplication of one obligation reported by both sources, peak debt service and TTM exposure, isolation of another Business's debt, conflicting/incomplete calendars, and source corrections preserving the old UUID receipt while invalidating current use. Full local proof: 2,248 non-TIA PHP tests / 11,684 assertions at 100% application lines; PHPStan zero; Pint clean. These remain isolated Alpha facts; S-A/S-B non-author review and the rolled S-C follow-ups remain open.
      - [x] Implement D-14's inclusive 45-day Kigali freshness rule with the current 36/qualified-12-month observation windows. A pure selector excludes current/future partial months, retains gaps/duplicates/unverified months for refusal, and selects the latest approved window without deleting older source history. Immutable quotes pin the window version and expiry; day 46 withdraws current signing authority, while a previously submitted application and old UUID receipts remain historical. An unsigned Review crossing the first of the Kigali month needs a fresh quote and new acceptance of that quote, without unnecessarily demanding the just-ended month's statements. Proof at `4af5f27d`: 84 focused tests / 619 assertions; 2,319 full non-TIA PHP tests / 12,275 assertions at 100% application lines; zero PHPStan errors; Pint clean; exact-commit web gate 940 tests across 81 files at 100% all four metrics, with types/lint/i18n/build/generated checks. [Erastus approved the freshness scope](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5313622833). Hosted checks and the combined integration gate remain separate; a distinct stale-evidence code is a non-blocking follow-up.
    - [x] **S-D:** Versioned procedure/ledger step saves, step-up, request-changes/reject/amend, actual co-signature, immutable report/seal and authorized publication/read. Delivery 3 is [independently approved for its scope at `b7d79c2b`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5319993028); combined browser and final-candidate gates remain J-1/J-2/J-3 requirements.
      - [x] Implement the protected submitted-application binding and explicit `audit.start` application action. Encrypted reports pin the exact immutable application version, submission, quote and mandate; database constraints preserve source ownership, terminal history and amendment lineage. Reads create nothing, same-UUID retries commit once, and current assignment/identity/standing authority gates every read/replay. A replacement Auditor cannot silently inherit the original draft. Proof: 2,382 full non-TIA PHP tests / 13,301 assertions at 100% application lines; zero PHPStan errors; six new PostgreSQL concurrency cases / 24 assertions; both report-model bypass probes rejected with all 12 Auditor targets and a green teardown; matching schema/module inventory. The start/save HTTP Resources, procedure completion, co-signature/seal/publication and S-D non-author review remain open. [Erastus owns the U-C adjustments](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5827500135); his [in-flight confirmation fix](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5827740344) remains on draft #113.
      - [x] Implement [Erastus's report-start review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5314309898) corrections 1–3 and the rollback nit. Conflict declaration now withdraws the former Auditor's draft and appends its immutable history atomically; immediate or Operations-led reassignment permits a fresh report pinned to the replacement's accepted assignment revision. The Business port and report binding retain only source IDs/revisions/hashes, without copied mandate terms, signer identities, scorecard or credit facts. PostgreSQL also requires the exact submitted application revision/quote/submission and current accepted author. Local correction proof: 2,395 full non-TIA PHP tests / 13,397 assertions at 100% application lines; zero PHPStan errors; eight report races / 32 assertions; all 12 Auditor model bypass probes rejected with green teardown; matching inventory and clean Pint. [Erastus approved the report-start correction at `d7efd07d`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5314584034); all five hosted checks pass on that exact head. His follow-ups retain report-only revisions for procedure saves, explicit amendment/returned-report transitions, current read locks and refusal semantics. Read-lock optimization, engagement binding, live procedure/Resource work and whole-C2 acceptance remain open.
      - [x] Implement retained Auditor platform MSA/MVP-AUP-1 terms and explicit Party acceptance as the FR-301 backend prerequisite. Full originals and canonical hashes are encrypted and immutable; compliance staff publish distinct revisions, and withdrawal never falls back to older terms. Dispatch, offer acceptance and protected evidence/report actions require current acceptance held through their transaction. New acceptance preserves assignment revisions and existing report identity; report/source snapshots retain the accepted agreement pins. Shared web/API JSON Resources provide full terms, strict explicit acceptance and same-UUID receipt recovery with current role/MFA/token checks. Local proof: 2,480 full non-TIA PHP tests / 13,749 assertions at 100% application lines; zero PHPStan errors; five engagement-related PostgreSQL tests / 19 assertions, including three new release/acceptance/withdrawal races; all 14 Auditor-model bypass probes rejected with a green teardown; matching inventory and clean Pint. Synthetic terms are restricted to isolated environments. Auditor agreement UI/browser integration, assignment-specific procedure completion, Business co-signature, seal/publication and non-author review remain open.
      - [x] Implement the [agreed engagement entry summary](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5828936059) and corrections 1–2 from [Erastus's `d24a6e95` review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5315181399). Auditor Home/Profile/Jobs/file expose only current/required/unavailable status and the matching web/API agreement link; other role homes return null. A valid retained release for another procedure becomes unavailable while actual corruption remains an integrity failure. Existing offers remain readable, decline/conflict remain available, and expiry/Operations continue. The guarded `auditor:engagement-terms` command publishes or withdraws full originals from bounded local JSON through current compliance authority, with explicit revision/UUID and nonzero refusal exits; UAT/production can accept non-synthetic input without enabling synthetic seeding. Local proof: 2,525 full non-TIA PHP tests / 14,312 assertions at 100% application lines; 85 final affected tests / 701 assertions; zero PHPStan errors; clean Pint and matching inventory. All five hosted checks pass at `79d9ef50dff46755fd6119ff201faa79c3234360`; its exact-commit web gate passes 1,014 tests across 84 files with all four metrics at 100% and D-67 over 321 authored files. [Erastus approved this engagement slice at that head](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5315485375). Erastus owns the agreement Inertia page/controller split, UI types/banners and browser proof; no UI completion, real terms publication or promotion is claimed.
      - [x] Implement engagement review items 3–4: report and source-verification rows now have immutable database-visible acceptance/author pins. Composite foreign keys reject foreign authors; insertion guards require the current active release and serialize with direct catalog publication. The atomic forward migration derives metadata only from verified encrypted historical pins, preserves originals/versions/revisions/timestamps after withdrawal, and rolls back completely when evidence is missing or inconsistent. Local proof: 2,554 full non-TIA PHP tests / 14,460 assertions at 100% application lines; zero PHPStan errors; ten engagement/publication/source concurrency tests / 43 assertions; all 14 Auditor persistence bypass targets rejected with green architecture teardown; clean Pint and matching inventory. This verified correction is included in the next finished S-D procedure delivery for review; its completion does not close S-D or C2.
      - Remaining engagement/read follow-ups are retained below under the [joint C2 exit assessment](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5834433775). Existing accepted-engagement/source-pin guards and reviewed re-accept-then-recover behavior remain enforced; this classification does not close unimplemented follow-ups.
      - [ ] Non-blocking follow-up list (not an additional C2 exit gate): Business/Jobs summary locks and initial file queries, stable offer-time cursors, explicit application/revision assignment binding and a legitimate next-engagement/completion trigger, engagement authority-read refinements and remaining release/hash recovery regression cases, lighter summary reads, durable publication-withdrawal conflict markers for direct SQL writers using isolation above the documented READ COMMITTED contract, distinct refusal-code names, decline/conflict behavior under actual catalog corruption, legal-text control-character feedback, publish-refusal ordering/clean CLI errors for missing actors or invalid host configuration, any separately approved non-material release policy, per-offer reason metadata and self-describing previous-acceptance metadata. Escalate an item only if a concrete correctness, security or data-integrity failure affects the delivered scope. The [final U-D handoff](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5835905122) also records a two-line browser abort adjustment for an intermittent temporary-branch harness stall and a separate timeout/Check again treatment for a nonresponding operation lookup. Neither affected the passing candidate browser runs; [both are retained as post-C2 follow-ups](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5836330701).
      - Delivery order [reconfirmed with Erastus](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5829515627): procedure/ledger plus live U-C Resources; request-changes/reject/amend; bound step-up/co-sign/seal/publication; then joint S-E/J-1–J-3 validation. Review each finished delivery once at its exact SHA and batch ordinary fixes. Track progress by completed, tested deliveries; the earlier unmeasured completion range is withdrawn. Erastus may prepare non-activatable C3 contracts/fixtures in parallel; C3 activation still follows C2 acceptance. Agreement UI [PR #114](https://github.com/rozine-rw/rozine/pull/114) is [independently approved](https://github.com/rozine-rw/rozine/pull/114#pullrequestreview-5315788155) at `8963df4624e95b5950aead2ab7564c10f870da90`, with all five hosted checks green. Its isolated review passed 43 PHP contract tests / 1,052 assertions, all 1,046 frontend tests across 87 files, the production build and the committed desktop/390×844 light/dark browser journey. An additional phone acceptance interaction passed and rendered captures were inspected. Original UI history is integrated into the checkpoint branch; 90 affected combined PHP tests / 1,471 assertions pass, including the retained source-pin migration. At `de54a56707a39fb58c9251bfc90ba3a0f96984c5`, the combined exact-commit web gate passes 1,046 tests across 87 files at 100% for all four metrics, D-67 over 325 authored files, types/lint/i18n/build/generated checks, and all five hosted checks. C3 stays in Erastus’s separate non-activatable scaffold, as [confirmed](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5829861411), while U-C awaits S-D delivery 1.
      - Procedure/ledger implementation is complete locally; delivery review remains open. Report-owned step saves retain immutable history and original UUID receipts, enforce ordered steps/current source pins, and leave assignment revisions unchanged. Shared web/API start/read/save/lookup and scoped original downloads project the dispatch-pinned reporting month, exact cash-flow totals and explicit unavailable facts. Read-only Back never writes history; changing an earlier step explicitly clears later completion. Ledger uploads retain report-owned originals and rescan references, leave Business verification and its evidence revision unchanged, and roll back pending extraction/original writes if report history fails; ingestion never approves or advances a procedure. [Erastus's isolated source adapter #116 at `dcb7dc0a`](https://github.com/rozine-rw/rozine/pull/116) and [#113 UI through `0e9af945`](https://github.com/rozine-rw/rozine/pull/113) are integrated with their history preserved. Source declarations, check-in/photo metadata and synthetic provenance now compose under current accepted-assignment authority; missing, withdrawn or changed facts withdraw readiness. Local combined proof: 2,753 full non-TIA PHP tests / 16,164 assertions, 100% application lines, zero PHPStan errors, matching generated inventory, clean Pint, and all 21 Auditor/evidence boundary bypass probes caught with green teardown. The three report-save PostgreSQL races pass (13 assertions), including competing commands and authority withdrawal through the actual version write. Both real Flash/monthly phone journeys now pass (2 tests / 10 assertions), including saved-note reload and readiness withdrawal after source changes; current phone and desktop captures were visually inspected. [Erastus’s mobile overlay correction `102af52a`](https://github.com/rozine-rw/rozine/commit/102af52a31f1b17a9ea96b6eb044187dbf02a186) preserves the monthly report when its live Jobs background column is absent. The browser harness waits for the preceding Inertia navigation before editing the note. The exact-commit frontend gate passes at `daa557d2f7a8ae07451f33e5ffd11262a7b31f9d`: 1,108 tests across 87 files, 100% statements/branches/functions/lines, D-67 across 325 authored files, plus lint/format/types/i18n/build/generated checks. Hosted checks and finished-delivery review remain separate gates. Synthetic source facts cannot satisfy D-04 or authorize live capture.
      - The [delivery-1 review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5317314045) found that the original ledger path wrote into Business statement evidence. The correction stores immutable encrypted ledger originals and extraction history under the report, pins active originals in the saved procedure and preview, scopes rescan replacement/download access to that report, and clears active references when an earlier step is corrected while retaining history. Business statements are labelled separately. Empty monthly proof lists withdraw readiness. Local proof: 2,779 full non-TIA PHP tests / 16,339 assertions at 100% application lines and zero PHPStan errors; this includes concurrent Business ingestion/reconciliation, unchanged Business verification across ledger uploads, real upload-to-preview progression, parser failures, rollback and current-engagement denial. [Erastus’s protected-original download UI `f39848d5`](https://github.com/rozine-rw/rozine/commit/f39848d55a1a6a54cadffe770e4737314d42a6d8) is integrated with its history. The combined Flash/monthly phone journeys pass (2 tests / 11 assertions), including an actual ledger upload, protected download with byte-for-byte equality, procedure continuation and saved-note reload. All 23 Auditor/evidence boundary bypass probes are caught with green teardown, and generated schema inventory matches. At published `7d073e3b09208f4de1e151d3d5b2bd69fb8f53f6`, the exact-commit web gate passes 1,110 tests across 87 files at 100% for all four metrics, with D-67 covering 325 authored files and all static/build/generated checks green. [Erastus accepted delivery 1 at that head](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5832481085), with his [review evidence](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5317686084); GitHub records the review as `COMMENTED`. All five hosted checks pass at that same delivery-1 head. The agreed ordinary follow-up batch includes explicit upload capability, a ledger-owner insertion guard, actual account MFA projection and the listed regression cases.
      - [x] Implement delivery 2's monthly request-changes/reject and linked-amendment commands, shared web/API Resources and live UI from [#113 `2e6b3fe6`](https://github.com/rozine-rw/rozine/pull/113), preserving its history. Decisions require a factual code and explanation, retain an immutable terminal version and leave the application/assignment unchanged. An amendment starts a fresh procedure against the same submitted source lineage, retains its parent and resumes the same child for competing UUIDs. PostgreSQL rejects mutable or mismatched parents. Current authority gates commands and recovered receipts; read-only tokens receive no actions. The returned/rejected phone journeys verify retained reasons, single-child navigation and fresh procedure state. Local focused proof: 427 PHP tests / 3,456 assertions, zero PHPStan errors, 1,137 web tests, and both real browser journeys (2 tests / 16 assertions), with rendered phone captures inspected. The older direct-publication race fixture now creates a valid returned parent before its direct child insert; its three race cases pass with 15 assertions. The complete non-TIA PHP gate passes 2,832 tests / 16,681 assertions at 100% application line coverage; generated schema/module inventory matches and Pint is clean. The exact-commit frontend gate passes at `9af24106eed5b1393585b62cc10db9025ae75b43`: 1,137 tests across 88 files, 100% statements/branches/functions/lines, D-67 across 327 authored files, plus lint/format/types/i18n/build/generated checks. Delivery 2 is published at `3aa138eb42ad288050576c5d3cc38e2a4d571620`; [Erastus approved delivery 2 for its scope](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5833016930), with [independent PostgreSQL and race evidence](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5318077940) and no blockers. GitHub records that review as `COMMENTED`; the concrete ordinary fixes are tracked below. All five hosted gates pass at the published delivery-2 head. Review of later corrections remains separate; this does not close S-D or checkpoint acceptance.
      - [x] Implement the agreed ledger follow-up batch with [#113 `93161ef5`](https://github.com/rozine-rw/rozine/commit/93161ef50e6b79ac1630c9769943aac4668119a5), preserving its UI history. The server supplies `stage.upload` only for the current editable Flash ledger and an authorized command token; saved-step reads and terminal reports retain downloads without upload controls. PostgreSQL now ties inserted originals to the report author and exact draft/step/revision while holding the report lock through commit. Current account identity supplies MFA confirmation. New regressions cover former-Auditor/successor upload and download denial, same/competing UUID uploads, direct insertion against report advancement, invalid historical ownership and protected rollback. The 26 affected feature tests / 180 assertions and three new concurrency cases / 14 assertions pass, PHPStan is clean, and the Flash/monthly browser journeys pass (2 tests / 16 assertions). The inspected phone Back view retains the original download and omits the file input. The complete non-TIA PHP gate passes 2,849 tests / 16,774 assertions at 100% application lines; generated inventory matches and Pint is clean. At `798ac78c587ada56b45b2f519c7e8020a208eb00`, the exact-commit web gate passes 1,140 tests across 88 files with all four metrics at 100% and D-67 over 327 authored files; all five hosted checks pass. [Erastus accepted the combined correction batch](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5318546769), with 353 affected PostgreSQL tests / 3,004 assertions and independent race verification.
      - [x] Correct delivery-2 review items 1–4: allow normalized multiline explanations with distinct field errors, journal otherwise unhashable malformed reasons without changing existing receipt fingerprints, align conflict permissions/action links, and enforce monthly-only returns plus fresh amendment status/revision/step in PostgreSQL. The 184 affected tests / 1,666 assertions pass. [Erastus’s tests-only `b877be89`](https://github.com/rozine-rw/rozine/commit/b877be8933da93658e126c87b39ed4ddfe428608) is integrated with its history; all nine authority/lineage/multibyte and cross-command race cases pass (73 assertions). The phone browser journey accepts and displays a multiline reason; both journeys pass (2 tests / 16 assertions), with current captures inspected. The combined non-TIA PHP gate passes 2,876 tests / 16,988 assertions at 100% application line coverage, with zero PHPStan errors, clean Pint and matching generated inventory. The combined correction is published at `798ac78c`; its exact web gate and all five hosted checks pass, and [Erastus accepted the delta](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5833635395). Read-query optimization remains a non-blocking follow-up.
      - Delivery 3 is published at `e7fe1d072a4dfd3121565560b78397d1c0ce8a4f`, followed by the manifest-fingerprint-only `ea4db99c465e5617a3d6eb971c2f883324cff31e`; independent review and checkpoint acceptance remain open. Bound five-minute authenticator proofs, retained ES256/JCS seals, independently attributed mandated Business co-signatures and the existing monthly by-the-7th publication window now serve shared web/API contracts. The signing adapter is isolated-environment only; no managed production key or N6 amendment is activated. Erastus’s co-sign UI through [#115 `d2b76740`](https://github.com/rozine-rw/rozine/commit/d2b767405b535653c33a6d3d3e73087cb3d41c98) and sealed-status correction [`db536130`](https://github.com/rozine-rw/rozine/commit/db53613097f927b646b6e165de33cf4301888ee9) are integrated with their history. The guest web verification page and API share exactly the five public seal facts. Local focused proof: 148 affected PHP tests / 1,146 assertions, then 53 sealing/publication cases / 329 assertions; all five new PostgreSQL signing/publication races pass. The actual phone signing-to-co-sign-to-publication journey passes (1 test / 5 assertions), with phone/desktop captures inspected. All 23 Auditor-model bypass probes are caught, followed by a green architecture teardown (24 successful control outcomes); PHPStan is clean, the production build passes and generated inventory matches. The complete non-TIA PHP gate passes 2,975 tests / 17,579 assertions at 100% application lines; the manifest-only follow-up changes no PHP/test source. At `ea4db99c`, the exact-commit web gate passes 1,205 tests across 90 files with 100% statements/branches/functions/lines and D-67 admission over 334 authored files, including all static/build/generated checks. All seven existing browser journeys pass together (35 assertions); current phone/desktop publication captures were inspected, including the corrected published intro. Erastus completed the [exact-code server review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5319593905), identifying one publication-after-amendment blocker and a bounded correction batch. The corrections are published at `b7d79c2b823e2ce4e9a4e040bf53c8a48ab15cd4` with 82 sealing/publication regressions (570 assertions), including three additional amendment races, and 97 migration-chain cases (648 assertions). They withdraw parent co-signing once any amendment exists, preserve prior signatures and published history, fix published-monthly overdue and web identity-refusal presentation, bind retained proofs to their seal and purpose, and use one publication instant. His matching UI correction `d318cce860c4478cc78fe797d696a8cebd01497e` is integrated with its history. The four signing-entry-point bypass probes are caught with a green architecture teardown; PHPStan is clean. The additional replay/conflict and review regressions pass (28 tests / 211 assertions). Erastus’s [browser handoff](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5835490837) adds six passing edge/recovery journeys and identifies two UI blockers: returning from a changed-source seal preview and the missing identity-context revision in co-sign lookup. Both UI fixes are integrated from `3fa8a0f7` and `9d7dacac`, with the enabled browser handoff `de1f888c` and its source history. Erastus approved the server correction scope in [review 5319993028](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5319993028); the READ COMMITTED requirement is now explicit in ADR-0001 and asserted in the PostgreSQL configuration test. The full corrected PHP run passed 3,007 tests / 17,805 assertions but found one uncovered missing-publication denial branch; a new passing regression now proves an Auditor draft cannot be read as a Business publication. The exact `b7d79c2b` web gate passed 1,206 tests with all four metrics at 100%. The final amended-parent presentation correction `280f23d0` is also integrated: an unpublished amended report explains its replacement, while a previously published report retains published wording. The all-enabled combined browser run, complete 100% PHP coverage, hosted CI and both-lane final-candidate acceptance remain open; this does not mark delivery 3 or the checkpoint accepted. The [fixed exit assessment](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5834433775) keeps the remaining work bounded to those gates.
    - [x] **S-E:** Live shared web and `/api/v1` contracts within each slice: `data.next`, same-UUID command/lookup, recorded 422 recovery, current authorization, response/record clocks and field errors.
      - [x] Verify the actual JSON wire types for direct Business commands and recovered refusals on both transports: empty error maps remain objects, absent outcome data is `null`, and populated data remains an object. This resolves [Erastus's U-A integration note](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5826408029); the [shared operation projection](../app/Http/Resources/OperationResource.php) and [HTTP regressions](../tests/Feature/BusinessApplicationHttpTest.php) are included in the 2,248-test local gate above.
    - [x] **U-A/U-B/U-C:** Erastus binds Business Apply, Auditor Jobs/file/conflict register, then procedure/seal/co-sign against the published slice contracts.
      - [x] Accept U-A at [#111 `45df0d2c`](https://github.com/rozine-rw/rozine/pull/111), with all five hosted checks green. The [independent re-review](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5827163010) ran 12 real-resource/browser tests / 697 assertions and 147 affected frontend tests across 7 files in an isolated checkout. Inspected desktop and 390×844 phone captures verify save/resume/requote, two-party signature/submission, pending-application links and withdrawn-evidence refusal. Recovered completions and stale refusals remount from current facts, and recorded 422 step refusals are handled. #111 and the reviewed #112 are integrated with original history preserved. Combined PHP proof at `ea9eb305`: 2,335 full non-TIA tests / 13,000 assertions, 100% application line coverage and zero PHPStan errors. The inventory-only correction at `d57be224` passes exact-commit web admission: 1,014 tests across 84 files, 100% all four metrics and D-67 over 321 authored files, plus types/lint/i18n/build/generated checks. All three committed identity, Auditor Jobs and Business Apply browser journeys pass together at that head; current desktop/phone captures were inspected, and the schema/module inventory matches. All five hosted integration checks pass at `7b129727`; whole-C2 acceptance remains open. Erastus owns U-C/U-D after the S-D Resource handoff; server read/engagement/pagination work remains Aminu's.
    - [x] **U-D:** Commit desktop/phone browser journeys for apply → quote → sign → submit; accept → procedure → seal; Business co-sign; denied/stale/retry/reload paths.
    - [x] **J-1/J-2:** Audit the fixed C2 exit list at the exact candidate and complete full PHP/web/static/browser/hosted gates and both-lane review. The final candidate evidence above covers S-A through S-E and U-A through U-D; the agreed non-blocking follow-ups remain explicitly unimplemented.
    - [x] **J-3:** Aminu explicitly authorized the merge, #101 is merged into `dev` at `bb0fa3204bf672d66043581fe76840fb7dc30cb9`, and checkpoint acceptance is recorded above. Checkpoint 3 starts separately on a new branch from the updated `dev`.
    - [x] **Audit walkthrough server follow-up, 2026-09-26:** Auditor home receives current Jobs/Profile links; sealed work shows awaiting co-sign, published work leaves the active list, sealed procedures link to verification, and Business home links to its own latest unamended sealed report. Stock tolerance uses units separately from cash. Ordinary browser route/model misses now render the neutral localized 404 contract. Eight dedicated regressions (145 assertions), the corrected strict UI fixture/preview suites (262 tests / 1,713 assertions together), 1,212 web tests, PHPStan, Pint and the production build pass. The full non-TIA PHP coverage run exercised 3,016 tests with 100% application statements; its 19 fixture/stale-build failures were corrected and all affected suites rerun successfully. The merged #122 base is integrated with its history. The actual phone browser journey now passes through Auditor Home → Jobs, seal verification, awaiting-co-sign status, human co-sign/publication, removal from active work, and the neutral 404 (1 test / 5 database assertions, plus browser checks); current phone captures were inspected. The review correction excludes published report leaves before Jobs pagination, with mixed-page/amendment and monthly/expired-Flash regressions (28 affected tests / 524 assertions). Assignment completion and dispatch capacity accounting remain separate follow-ups. Unmatched URLs use the public 404 fallback because they have no web session middleware. Erastus approved the corrected code at `794a9ba3` and the unreachable-filter removal at `67734221` in [review 5326020709](https://github.com/rozine-rw/rozine/pull/124#pullrequestreview-5326020709). The store alone filters published jobs before pagination. Hosted CI and merge remain open.
    - [x] **Local manual-testing handoff, 2026-09-26:** Added the opt-in `CheckpointTwoSeeder` and `local:checkpoint-two` command on `codex/checkpoint-two-manual-test-pack`. Applied the 32 pending local migrations and verified 21 synthetic accounts, five applications, three audit reports and one published report while preserving all three existing Pulse signups. Reruns preserve progress and refresh report links; separate authenticator codes retain real MFA and step-up. The [manual walkthrough](Checkpoint_Two_Manual_Testing.md) documents the pre-verified statement history, two-signatory case, seal/co-sign/publication cases, sample CSV, 24-hour Flash deadlines and current Operations JSON boundary. Evidence: 116 affected PHP/architecture tests, the seeded application-to-publication browser journey (1 test / 7 assertions), and 1,212 web tests pass. Local Business/Auditor/Operations logins were checked. These local additions do not start checkpoint 3 or claim a new merge/deployment.
      - [x] Publish [PR #123](https://github.com/rozine-rw/rozine/pull/123) and address Erastus’s non-blocking review: hide the guarded local command, refresh the baseline inventory, support a separate local `rozine_manual` database, use named routes, omit predictable recovery codes, and verify independent MFA secrets plus the engagement-release refusal. The correction passes 41 affected PHP/architecture tests / 257 assertions and PHPStan. Erastus approved `f522d53f`; all five hosted checks passed and #123 is merged into `dev` at `e68a07b0`.
    - [ ] **N6 monthly review amendment, 2026-09-26:** Implement the approved 24-hour review window for new monthly seals with one verified mandated signer, undisputed timeout approval, retained private dispute proof, CPA review, explicit staff escalation/resolution and linked amendments. Legacy monthly and Flash publication rules remain retained. The server implementation and #125 UI are integrated with #124 and merged #123 history; final combined coverage, independent review and promotion remain open.
      - [x] Implement shared web/API commands, receipt recovery and protected original downloads; current authority checks and PostgreSQL decision history prevent competing outcomes. Focused monthly, transport, legacy, refusal and concurrency regressions pass. The real phone journey passes Business dispute/upload/reload/original download → CPA MFA/uphold → staff publication with zero invented signatures (1 test / 6 assertions plus browser checks). Inspected captures identified note-attribution/header copy corrections, handed to Erastus on issue #96. The two source lanes are still under integrated validation; checkpoint 3 is not started.
      - [x] Address [Erastus’s N6 server review](https://github.com/rozine-rw/rozine/pull/126#pullrequestreview-5325881016): allow timely Business objections during CPA/source outages; freeze amendment-required decisions in commands, Resources and database guards; fetch proof metadata without loading encrypted originals; verify attributed event hashes and their predecessor chain; expose blocked expiry in Operations; and correct monthly walkthrough expiry. The correction is published at `db22f898` with #125 `f4b16769` and #124 `67734221` history preserved. Twenty additional HTTP/integrity/review regressions pass (111 assertions), PHPStan is clean and Pint passes. Its exact web gate passes 1,308 tests with all four metrics at 100% over 340 authored files, plus types, locale, build and generated-route checks. Two actual browser journeys pass (11 database assertions plus browser checks); corrected staff publication, CPA/staff note attribution and no invented co-signature are verified in inspected phone/desktop captures. The complete corrected non-TIA PHP gate passes 3,132 tests / 18,691 assertions at 100% application lines. Hosted checks and independent correction acceptance remain pending. Staging/production upload limits and cross-table TRUNCATE hardening are recorded follow-ups, not deployment claims.
  - [x] Establish scoped staff operations, current verified entity/mandate authority and versioned consent; preserve person-level role isolation and prohibit tax identifiers. Isolated C2 implementation is covered by the accepted S-A/S-B slices, [authority tests](../tests/Feature/BusinessAuthorityTest.php), [staff tests](../tests/Feature/StaffAccessTest.php), [consent tests](../tests/Feature/ConsentCatalogTest.php) and the complete 2,975-test gate. Actual C6 legal approval remains separate.
    - [x] Implement explicit staff permissions and canonical RDB organization identity resolution with current verified-party guards. Local evidence: 36 focused feature tests / 210 assertions; PostgreSQL identity concurrency suite 10 tests / 35 assertions, including same-organization resolution, duplicate requests and locks against staff/entity/person revocation. Full mandate/consent workflows remain open.
    - [x] Add versioned Business mandates with the actual required signer set, explicit action permissions, RDB binding, effective dates and immutable profile/authority history. Business/organization feature checks pass 58 tests / 208 assertions; the combined identity, journal and mandate PostgreSQL suite passes 15 tests / 51 assertions, including competing mandate revisions and revocation during protected work. Consent and live application transports remain open.
    - [x] Add immutable consent releases with complete original Terms/Privacy text, summaries, disclosure versions and server-computed hashes, explicit compliance permission, withdrawal without fallback, exact acceptance matching and fixture-environment enforcement. The current release is locked through protected acceptance; competing publication and withdrawal are covered in the 17-test / 58-assertion PostgreSQL concurrency suite. The full PHP suite passes 1,250 tests / 6,262 assertions at 100% application line coverage, with zero PHPStan errors. Identity negative controls reject direct consent-history writes. Application signature receipts and actual C6 approved legal text remain open.
  - [x] Implement resumable Business onboarding/application actions, shared web/API Resources and live Business screens using deterministic verification/provider adapters for isolated Alpha. Accepted S-A and U-A evidence above is retained; [HTTP parity tests](../tests/Feature/BusinessApplicationHttpTest.php) and the [real Business journey](../tests/Browser/BusinessApplyBrowserTest.php) also pass in the integrated delivery-3 suite.
    - [x] Add the shared command journal and operation Resource: 26 feature tests / 87 assertions; 13 combined identity/journal PostgreSQL concurrency tests / 44 assertions. Same-Party retries across logins commit once, changed bodies conflict, current authorization gates replay/lookup, rejected effects roll back, and unknown/in-flight work is not misreported as a committed result. Identity, domain and operation protected-boundary negative controls pass. Live transports are not yet registered.
    - [x] Persist resumable Business/Raise drafts and append-only application versions through current Business authority. Creation/save/lookups share the journal, exact revisions reject competing edits, old retries cannot restore superseded input, and reads do not mutate drafts. One open draft per Business is enforced in PostgreSQL; concurrent create requests resume the same draft without rewriting its history, and Home can discover the current draft through a read-only application action. The full non-TIA PHP suite passes 1,413 tests / 6,808 assertions at 100% application line coverage; PHPStan has zero errors. The 22-test / 79-assertion PostgreSQL suite covers concurrent creates, same-Party saves across logins and competing mandate holders. Review-step advancement, immutable quotes, signatures and live transports remain open.
  - [x] Retain original statements and hashes, parse and reconcile all declared rails, append corrections and immutable verified monthly snapshots; expose parsing feedback and actionable denials. The accepted evidence/S-A/S-B slices and [statement evidence](../tests/Feature/StatementEvidenceTest.php), [transcription](../tests/Feature/StatementTranscriptionTest.php) and [verification tests](../tests/Feature/StatementVerificationTest.php) pass in the complete integrated suite. Unsupported or unreadable extraction remains an explicit refusal, without an OCR or live-provider claim.
    - [x] Retain exact encrypted PDF/CSV originals with server hashes and immutable extraction lineage through current Business authority, revision checks and the shared journal. Read-only manifests expose metadata, and authorized original reads verify integrity. PDF text and structured CSV extraction return explicit review feedback without fabricated financial values; every receipt remains `INGESTED_NOT_AUDIT_APPROVED`. Full PHP evidence: 1,339 non-TIA tests / 6,618 assertions at 100% application line coverage, zero PHPStan errors, and three evidence-boundary bypass probes caught with clean teardown. Bank-format mapping, verified monthly snapshots and live uploads remain open.
    - [x] Implement pure exact-RWF reconciliation of complete declared rail/month transcriptions: source membership, opening/closing continuity, duplicate conflicts, separate operating/financing/transfer/draw/debt classes, same-month returned draws and specifically authorized one-off exclusions. The output is always unverified. Evidence: 70 focused tests / 156 assertions; full non-TIA PHP suite 1,409 tests / 6,776 assertions at 100% application line coverage; zero PHPStan errors. Authoritative source verification, Auditor approval, immutable verified snapshots and live transport remain open.
    - [x] Persist encrypted, canonical-hashed reconciliation transcriptions and append linked correction versions through current Business authority. Bind the selected original hashes and classification version, preserve old records/retry results, and mark prior transcriptions stale after new evidence. Every output remains unverified; complete inventory and source authenticity still require Auditor approval. Evidence: 1,426 complete PHP tests / 6,894 assertions at 100% application lines, zero PHPStan errors, 24 PostgreSQL concurrency tests / 86 assertions, and all four evidence-boundary bypass probes caught with clean teardown. Live transport, factual approval and verified monthly snapshots remain open.
    - [x] Expose assignment-scoped statement manifests, exact originals and correction history to the currently accepted Auditor through a shared protected action. Business/mandate, actor/MFA, standing, independence and assignment locks remain held through the read; offered, unrelated, reassigned and revoked actors cannot read private evidence. Original filenames are replaced by neutral download names, hashes are checked, and stale/current transcriptions remain explicitly unverified. Proof: 82 focused tests / 394 assertions; 1,764 full non-TIA PHP tests / 8,371 assertions at 100% application lines; zero PHPStan errors; 47 PostgreSQL concurrency/worker tests / 178 assertions, including protected reads against simultaneous conflict, standing, membership and evidence changes. This enables factual review but does not complete verified monthly snapshots, live transport, professional findings or seals.
    - [x] Validate and cross-review the PR #101 evidence corrections: durable pending ingestion and bounded parser workers; matched transfer legs and separate financing/transfer totals; zero opening/closing at mid-window rail boundaries; private original/history access and encrypted filenames. Local proof: 1,472 full non-TIA PHP tests / 7,047 assertions at 100% application lines, zero PHPStan errors, 32 PostgreSQL concurrency/worker tests / 123 assertions, matching baseline inventory and four evidence-boundary bypass probes caught with clean teardown. Hostile-PDF, maximum-size input, migration, retry and classification regressions pass. All five hosted checks pass on `6f49af0add7ec10ef1d45ae1269eae912122fe5d`; [Erastus approved that foundation](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5302376927), resolving B1/B2 and the privacy findings. Live HTTP/FPM acceptance and verified evidence publication remain open.
    - [x] Persist immutable factual source-verification snapshots through the currently accepted, qualified Auditor. Bind original hashes, the current reconciliation, exact revisions, reviewed complete rail inventory, all seven source checks, evidenced debt schedules/draws, accreditation and mandate facts under a canonical hash. Corrections append history; new evidence, changed Business authority or a later conflict invalidates the current snapshot without rewriting history. Verified monthly facts feed the existing underwriting evidence calculator; a persisted 36-month example proves the path. Receipts explicitly remain `not_cosigned`: final report procedures, co-signature, publication/seal and live binding remain open. Local proof: 1,816 full non-TIA PHP tests / 8,558 assertions at 100% application lines; zero PHPStan errors; 51 PostgreSQL concurrency/worker tests / 188 assertions; all five evidence-model bypass probes caught with clean teardown. The [snapshot review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5305525634) currentness and classification findings B1/S1 are corrected: withdrawn standing/identity/membership invalidates current eligibility, ordinary expiry/pause/pending-renewal withdrawal preserves historical validity, classification upgrades invalidate current use, and reads bind the actual transcription digest. S2 now returns the conflicted revision; corruption remains an unjournaled server error. New proof: 36 focused tests / 147 assertions; 1,873 full non-TIA PHP tests / 9,179 assertions at 100% application lines; zero PHPStan errors; 58 PostgreSQL concurrency/worker tests / 207 assertions, including simultaneous transcription, independence, standing and membership changes. [Erastus approved B1/S1, the revision/integrity corrections and coded declines at `06cd87c2`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5305783823). S3/S4 are now implemented locally: nested review shapes are validated before access; snapshots bind the normalized mandate hash, independence-review provenance, standing and per-original extraction lineage; later parser output preserves earlier manual review. S5 adds a replacement-Auditor amendment test alongside the recorded races. Complete proof: 116 focused tests / 385 assertions; 1,912 full non-TIA PHP tests / 9,280 assertions at 100% application lines; zero PHPStan errors; 58 PostgreSQL concurrency/worker tests / 207 assertions. The exact `06cd87c2` web gate passed (899 tests; 100% all four metrics; D-67 315 files), but its hosted PHP gate exposed a wall-clock test flake, corrected here by freezing the test second. [Erastus approved S3/S4 and the clock fix at `944e7f92`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5305968502); all five hosted gates and the exact-commit web gate passed (899 tests; 100% all four metrics; D-67 315 files). The dedicated replacement-Auditor amendment race is included in the S-C evidence below. Evaluation/live binding and whole-checkpoint acceptance remain separate work. Live HTTP/browser acceptance remains open.
  - [x] Implement the current engineering-2026-09-23.4 exact-money scorecard, pricing, 36/12-month evidence rules, initial-request cutoff, final DSCR, repeat projections, exposure and RWF 5,000 offer quantization, with immutable replayable calculations and acceptance authority. Accepted S-B evidence and [persisted underwriting regressions](../tests/Feature/BusinessUnderwritingPersistenceTest.php) pass in the complete integrated suite; canonical live servicing/ledger inputs remain C3/C4 work.
    - [x] Implement the pure exact-rational calculator and approved arithmetic vectors: 66 focused unit tests / 250 assertions, with all new domain lines exercised. This covers pricing, capacity, scorecard and verified-month arithmetic; source verification, persisted calculations and acceptance remain open.
    - [x] Obtain Aminu’s explicit dependency approval and install PDF parser 2.12.5, JWT library 4.2.3 and JCS 1.0.0; declare existing brick/math 0.18 directly. Composer resolved three additions, no existing-version changes, and no known advisories.
  - [ ] Implement Auditor accreditation/standing, conflict-safe dispatch/assignment, procedure/evidence review, factual findings, co-signature and immutable seal through the online Alpha chain; retain separate native/offline and external-evidence gates.
    - [x] Implement the pure current-standing, deterministic dispatch and assignment-clock rules. Deny stale/expired/revoked standing, incomplete location verification, capacity/rotation/conflict violations and uncertain radius admission; select by active count, oldest assignment and stable ID. Flash reassignment preserves its original deadline, and exhausted attempts route to Operations. Evidence: 59 focused tests / 134 assertions and the full non-TIA PHP suite at 1,556 tests / 7,338 assertions with 100% application lines; zero PHPStan errors. All five hosted checks pass at `d138a2349e328eb5b3832d675bbfb34ec41d427d`, and [Erastus approved the policy slice](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5302548428). Persisted accreditation/assignment actions, authoritative geodesic calculation and live commands remain open.
    - [x] Persist Auditor accreditation claims, staff-evidenced standing and availability through current role/MFA authority, shared revision checks and immutable operation receipts. Exact private certificates/filenames and profile history are encrypted; renewal or rejection cannot replace existing approved standing, and withdrawal defeats stale approval. Local proof: 1,616 full non-TIA PHP tests / 7,601 assertions at 100% application lines, zero PHPStan errors, 34 PostgreSQL concurrency/worker tests / 134 assertions, matching inventory and all three Auditor-model bypass probes caught with clean teardown. [Erastus approved the persistence slice at `9093c8ad`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5302887071). Live Resources/routes, expiry presentation and assignments remain open.
    - [x] Add the authorized accreditation read model and shared Resource with fresh `standing.current`/`reason`, Rwanda-calendar expiry, explicit action permissions, saved availability preference and private certificate hash metadata. Internal check references and audit notes are excluded; a certificate expiring before staff approval returns a recorded 422 validation error. Evidence: 47 focused tests / 239 assertions, 1,625 full PHP tests / 7,656 assertions at 100% application lines and zero PHPStan errors. All five hosted checks pass at `54b5fa6de533a50493d1c4c4f8d60c21ff0761a2`, and [Erastus approved that projection](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5303080631). His matching Home/Profile UI is integrated from [PR #106](https://github.com/rozine-rw/rozine/pull/106) at `22b8480383b7a68b3c56e4817d7f6180bf0056c2`, preserving green source `9b3a1e0f`; receiving work requires both the saved preference and current standing. The combined exact-commit web gate passes at `81f514c9786b62eb54462d68e6fa92fce37dddc0`: 895 tests across 79 files, 100% lines/statements/functions/branches and D-67 admission across 315 files, plus types/lint/format/i18n/build/generated-route checks. Live routing, command `data.next` and browser proof remain open.
    - [x] Persist staff-reviewed office/premises locations with current authority, encrypted immutable history and move invalidation, and compute conservative WGS84 distance on the server. Both uncertainty radii remain part of admission; nonconvergent distance is denied. Evidence: 98 focused tests / 410 assertions; 1,675 full non-TIA PHP tests / 7,918 assertions at 100% application lines; zero PHPStan errors; 36 PostgreSQL concurrency/worker tests / 143 assertions, including identical verification retries and move-versus-verification; all five Auditor/location-model bypass probes caught with clean teardown, and matching generated inventory. Implementation is committed at `81f514c9786b62eb54462d68e6fa92fce37dddc0`, and [Erastus approved the location boundary at `0e360afe`](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5303402373). His same-second timestamp note is corrected in both location verification and dispatch: a new check must be strictly after the recorded move. The correction passes 53 focused tests / 205 assertions and 1,676 full PHP tests / 7,921 assertions at 100% application lines, with zero PHPStan errors and matching inventory. All five hosted checks pass at `2c74dbf689a36565f2cfd7e9df33491f5bbd76e2`, and [Erastus approved the timestamp correction](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5303459610). Staff premises verification locks the current Business mandate and verified entity/people. Assignment binding, participant move reporting, live transport and browser proof remain open.
    - [x] Persist evidenced Auditor independence reviews per Business with encrypted immutable history, current Operations permission, mandate-version binding and no false-default clearance. A submitted false flag cannot clear a current mandate relationship. Local proof: 98 focused tests / 455 assertions; 1,703 full non-TIA PHP tests / 8,071 assertions at 100% application lines; zero PHPStan errors; 38 PostgreSQL concurrency/worker tests / 150 assertions, including identical reviews and crossed Business/Auditor identities; all seven Auditor-model bypass probes caught with clean teardown and matching inventory. All five hosted checks pass on `cbe70941011fd2f3f72676013b7510098fac02d8`, and [Erastus approved the independence boundary](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5303710507). Assignment integration and independent financial/role/declaration sources remain separate work.
    - [x] Persist audit offers, acceptance, declines and blocking conflicts with encrypted immutable history, server-resolved eligibility and one open engagement per Business. Current staff/actor/MFA, Business mandate, Party membership, standing, reviewed conflict facts, mandate history, registered coordinates, actual active load and completed-engagement rotation are checked under ordered locks. Conflict/decline withdraws private access and automatically reoffers; a minute worker advances expired offers without impersonating staff, and original Flash deadlines/three-attempt limits survive reassignment. Minimal current-authority receipts remain retrievable after withdrawal without exposing the replacement Auditor. Local proof: 36 assignment feature tests / 156 assertions; 1,744 full non-TIA PHP tests / 8,264 assertions at 100% application lines; zero PHPStan errors; 43 PostgreSQL concurrency/worker tests / 165 assertions, including two logins competing for the last capacity slot. All ten protected Auditor-model bypass probes are caught with clean teardown, and the generated inventory matches. All five hosted checks pass at `35882c9ee3076fd13bdd2451561061d74ab08db5`; Erastus has been requested to review that exact head. Operations resolution, sealed-report completion, live assignment transports and browser acceptance remain open. Existing external-conflict reviews do not replace a future authoritative platform-holdings check. **Profile integration correction:** Erastus’s [PR #108](https://github.com/rozine-rw/rozine/pull/108) identified that submit and renewal commands were not enforcing their advertised action distinction. The protected domain now records `ACCREDITATION_ACTION_NOT_ALLOWED` for the wrong command without changing approved facts; historical receipts still replay unchanged. Proof: 48 focused tests / 251 assertions, 1,745 full PHP tests / 8,276 assertions at 100% application lines and zero PHPStan errors. Erastus corrected the merge and page-error handling in PR #108 at `5991c7e20546acaa578f1441877b775617feb26d`; all five source checks pass. That source is integrated into the checkpoint branch with its original history. The combined Profile HTTP/source-verification tests pass (42 tests / 555 assertions), and the merged inventory matches. Combined final gates and browser acceptance remain open.
    - [x] Correct the first five assignment findings from [Erastus’s review](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5304861890): pin PostgreSQL to UTC and use immutable explicit offer instants; retain a private 409 expiry response for a former expired recipient; reject late declines; isolate/log individual expiry failures; and limit participant read/acceptance locks to that Auditor. Acceptance still serializes the three-active-engagement cap across logins/Businesses. Late authorized source reads remain available for remediation without extending deadlines or marking reports complete. Unrelated actors receive 404 before Business-authority details. Proof: 1,848 full non-TIA PHP tests / 9,079 assertions at 100% application lines; zero PHPStan errors; 54 PostgreSQL concurrency/worker tests / 199 assertions, including expiry-versus-acceptance, unrelated-Auditor updates during protected reads, cross-Business conflict/dispatch, and the existing last-capacity-slot race. The exact-commit web gate passes at `ff9700c4b46be42e7e51151d07e26aa81141c267` (899 tests; 100% all four metrics; D-67 315 files). [Erastus verified items 1–5 and accepted the documented capacity/remediation policies](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5305453932). Own-conflict projections, scoped Jobs lists, Operations resolution and report completion remain open.
    - [x] Accept server-validated decline codes `unavailable`, `capacity`, `location` and `other`, require a factual explanation only for `other`, preserve the code in the immutable receipt and bind it into retry identity. Unknown/missing choices and malformed explanations return recorded field errors without reoffering. Proof: 92 affected feature tests / 455 assertions; 1,856 full non-TIA PHP tests / 9,135 assertions at 100% application lines; zero PHPStan errors. Own-conflict projections, scoped Jobs lists and their live binding remain open.
  - [x] Connect the existing 1B Business/Auditor surfaces to authorized Resources/commands; verify stale, denied, retry, reload and responsive browser journeys. Erastus's original Business commits from [PR #102](https://github.com/rozine-rw/rozine/pull/102) are integrated at `8086d449ba1206f823704dfc5461b72083027c6a`, preserving history. That head passes all five hosted checks, 1,257 PHP tests / 6,304 assertions at 100% application lines, and exact-commit web admission (762 tests, all four metrics at 100% over 307 files). His [PR #103](https://github.com/rozine-rw/rozine/pull/103) follow-up adds full readable documents, explicit saved resume transitions and lower-principal preview; `offered_principal` is confirmed as the maximum server offer alongside the currently quoted `principal`. Erastus [fixed its generated-route mismatch at `2a8f64b1`](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5809338444), with all five hosted checks passing. It is integrated at `3d929a6ace95b9bd9e186ad4c2cbf0c82fb377a1`; the combined exact-commit web gate passes 768 tests, 100% all four metrics over 307 files, and types/lint/format/i18n/build/generated checks. His Auditor [PR #104](https://github.com/rozine-rw/rozine/pull/104) is integrated from green `d512273833555563bc02c22bd225a6cd7b1a31d8`, preserving all original commits. It includes per-record permissions, a separate conflict command flow, guarded late responses after access withdrawal, multipart operation receipts, current Portfolio revisions, Home application creation, visible field errors and real-transport recorded-422 recovery. The combined exact-commit web gate passes at `6f49af0a`: 878 tests, 100% all four metrics over 314 files, plus types/lint/format/i18n/build/generated checks. Its integrated fixture set passes 1,497 PHP tests / 7,197 assertions at 100% application lines, and all five hosted checks pass. Erastus's green [PR #105](https://github.com/rozine-rw/rozine/pull/105) is integrated from `f99134cb` at `47495a9f96178e2964dc9a374b6adfe76ef8fd16`. It aligns offers with server `accept_by` and original Flash `complete_by`, hides acceptance at expiry and preserves the separate Routine/monthly clocks. Combined proof: 1,617 PHP tests / 7,607 assertions at 100% application lines, and 885 web tests / 100% all four metrics / D-67 315 files with all web checks passing. All five hosted checks pass at `47495a9f`. A three-minute check of issue #96 and related PR reviews is active during checkpoint 2. Live routes and browser acceptance remain open.
  - [x] Freeze concrete protected-module boundaries in the ADR, prove negative controls and PostgreSQL races, pass full exact-commit PHP/web/CI gates, and audit every checkpoint 2 requirement before acceptance. **Published foundation:** [PR #101](https://github.com/rozine-rw/rozine/pull/101) at `1740ea9f355e1d73aaeb8bdc4cf63d9f7ebd01bb` passes all five hosted checks, 1,196 non-TIA PHP tests / 6,101 assertions at 100% application line coverage, all 28 negative-control scenarios, and exact-commit D-67 web admission (727 tests; all four metrics at 100% over 304 files). Erastus [reviewed that commit with no blockers](https://github.com/rozine-rw/rozine/pull/101#issuecomment-5808680302); [contract decisions](https://github.com/rozine-rw/rozine/pull/101#issuecomment-5808946760) retain the journal minimum without unsafe expiry/deletion, make all-of mandate scope explicit, and add a fresh response clock plus immutable `recorded_at` and Laravel `errors`. The subsequent consent slice has local evidence above; `vp check` remains at zero errors / 41 existing warnings. The foundation is now ready for review; the published source/draft slice at `1e283f02662a6001b71141ccbd4d47bec13d5b13` passes all five hosted checks and [Erastus has been requested to review it](https://github.com/rozine-rw/rozine/pull/101#issuecomment-5810450778). Erastus reviewed the consent/draft slice at `08a9ad09` without foundation blockers and identified live-binding work; [answers and ownership are recorded on issue #96](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5810019003). Erastus’s later review of `1e283f0` identified PDF resource exhaustion and unmatched transfer classifications; corrections at `57a1ef5609c0bce0ba92ec1245815028897e5935` have the local evidence above and [Erastus’s approval at the integrated `6f49af0a` head](https://github.com/rozine-rw/rozine/pull/101#pullrequestreview-5302376927). Whole-checkpoint acceptance remains open.
- [ ] Integrate checkpoint 3: wallet, Investor purchase, ledger, primary settlement, and Admin disbursement.
  - [ ] S3-A: final Business acceptance/exposure, staff release, campaign publication/progress/cancellation. Server work started from `dev` `f2cc72ce`; Erastus retains UI/browser ownership ([coordination](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5849001511)).
    - [x] Implement the AC-02 final-signature reservation boundary: one full-principal reservation in the submission transaction, immutable parent bindings and encrypted evidence, exact accepted-exposure arithmetic/deduplication, quote freshness, replay safety, and no inferred reservation for historical C2 submissions. Local proof: 3,164 complete non-TIA PHP tests / 18,806 assertions at 100% application line coverage, including the architecture suite, PostgreSQL concurrent signatures/retries, web/API receipt/reload parity and migration regressions. PHPStan reports zero errors; all nine Business-boundary negative controls are caught with a green teardown; the full web gate passes 1,308 tests at 100% for all four metrics. The final typed legacy test double is separately rechecked. [PR #132](https://github.com/rozine-rw/rozine/pull/132) remains draft pending exact-head hosted gates and non-author review; this does not complete S3-A.
    - [ ] Add staff `applications.review` release using retained acceptance/reservation and current engine/report/authority gates; freeze the shared Resource before live UI binding.
    - [ ] Add publication using the same exposure ID, campaign progress/restrictions, and cancellation with deterministic exposure release/refunds; verify the Business/Admin browser journey jointly.
  - [ ] S3-B: ledger-derived wallet balances, deposit policy/intents/outcomes and history.
  - [ ] S3-C: deals, exact unit reservations, purchase confirmation/cancellation, full-funding lock and expiry refunds.
  - [ ] S3-D: staff maker/checker disbursement, worker prechecks, provider reconciliation and verified-success issue.
  - [ ] Joint C3 close: exact-candidate complete gates, requirement audit, reciprocal review and browser evidence.
