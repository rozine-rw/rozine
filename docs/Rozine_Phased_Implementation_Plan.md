...
- [ ] Integrate checkpoint 2: Business application, statement parsing, underwriting, and Auditor filing. **IN PROGRESS — implementation started on `codex/phase-1-checkpoint-two`, based on the merged checkpoint 1.**
  ...
  - Fixed checkpoint 2 exit list: [Erastus’s proposal](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816417397) and [confirmed order/clarifications](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5816458617). Execute S-C → S-A/S-B → S-D, with S-E in each slice; review finished slices at exact SHAs and start matching UI binding immediately. Raise listing/Investor execution stays outside C2. Co-signed audit-report persistence/publication stays inside S-D. Online isolated Alpha does not satisfy the separate D-04, C6 or operational/provider release gates.
    - [x] **S-C:** Own-conflict receipts/register, scoped offers and assignments, authoritative allowed actions/clocks and naming maps, minimal staff redispatch/close, and their web/API Resources. **Accepted slice:** Erastus approved the corrections at `eac5d4f450c32c5d750d3a97dc070567fd47e350`; all five [hosted gates](https://github.com/rozine-rw/rozine/actions/runs/36080748059) passed on that exact head. This closes S-C only; the agreed follow-ups below remain in S-A/S-B and C2 remains open.
      ...
      - [ ] Finish the remaining agreed S-C follow-ups within S-A/S-B: reduce Business/Jobs read locks and background reloads; bind each assignment to an explicit application/revision and legitimate next-engagement trigger; use offer-time ordering with a stable cursor. Erastus owns [#112](https://github.com/rozine-rw/rozine/pull/112); he accepted the [deadline-during-read and hidden-reconnect findings](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5826785916) and is correcting them. The ADR records current-status conflict semantics and recorded versus pre-journal validation errors.
    ...
    - [ ] **S-B:** Persisted replayable underwriting bound to verified evidence and acceptance authority, including approved exact-money/evidence-window/exposure rules.
      ...
      - [x] Implement D-14's inclusive 45-day Kigali freshness rule with the current 36/qualified-12-month observation windows. A pure selector excludes current/future partial months, retains gaps/duplicates/unverified months for refusal, and selects the latest approved window without deleting older source history. Immutable quotes pin the window version and expiry; day 46 withdraws current signing authority, while a previously submitted application and old UUID receipts remain historical. A repayment-month change still requires a fresh quote, without unnecessarily demanding the just-ended month's statements. Local proof: 84 focused tests / 619 assertions, including first/repeat day-45 signing, day-46 refusal, leap/year/Kigali boundaries and selection of the repeat window from its retained baseline; 2,319 full non-TIA PHP tests / 12,275 assertions at 100% application lines; zero PHPStan errors; Pint clean. Hosted/exact-commit gates and follow-up review remain separate from this local evidence.
    ...
    - [ ] **U-A/U-B/U-C:** Erastus binds Business Apply, Auditor Jobs/file/conflict register, then procedure/seal/co-sign against the published slice contracts.
      - U-A review continues at [#111 `fc8024bf`](https://github.com/rozine-rw/rozine/pull/111). Erastus's [recovery correction](https://github.com/rozine-rw/rozine/issues/96#issuecomment-5826831074) replaces state-preserving reloads with non-preserving visits for recovered completions and stale refusals, pauses commands during the read, and adds the pending-application link plus recorded 422 step handling. Source inspection confirms the original reload defect is addressed; combined real-response/browser verification at the correction head remains open. Erastus owns #111 and the Jobs background-refresh follow-up #112; server read/engagement/pagination work remains Aminu's.
