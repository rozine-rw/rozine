...
`IngestStatement`, `GetStatementEvidence`, `ReadStatementOriginal` and `FindStatementOperation` under `App\Application\Evidence` use the `StatementStore` port. Its `App\Infrastructure\Evidence\EloquentStatementStore` adapter is the only context allowed to reference `StatementEvidence`, `StatementOriginal` and `StatementExtraction`, apart from model relationships and factories. Architecture rules assert these concrete targets exist; the `evidence-boundary` negative control plants bypass writes against each model and requires failure followed by clean teardown. The new protected seam requires non-author review with this implementation PR.

Ingestion holds current Business authority before the evidence aggregate and journal. It requires `application.save`, resolves the Party on the server, compares the expected evidence revision, and journals the original-byte digest rather than source contents. Repeated operations commit once; duplicate bytes within the same Business do not create another original or advance evidence. New source bytes append a revision. Original bytes and extracted text use Laravel encrypted casts in private database columns so the receipt, source and extraction commit atomically. PostgreSQL rejects original/extraction updates and deletes. Authorized source reads verify the stored digest and byte length; scoped manifests select only metadata and do not load or expose source/extracted text. There is no public storage URL or automatic purge.

`StatementSource` bounds originals to 10 MiB, validates plain PDF/CSV filenames and supported bytes, and computes SHA-256 on the exact input. `StatementTextExtractor` binds to `PdfAndCsvTextExtractor`; the approved PDF parser extracts text with image retention disabled and a bounded deflate buffer. UTF-8 CSV extraction preserves exact text, handles quoted multiline records and rejects inconsistent structures. Text extraction is bounded to 2 MiB. Unreadable/scanned/unsupported structures produce explicit review feedback while retaining the original. Successful ingestion is always `INGESTED_NOT_AUDIT_APPROVED`: no bank-format mapping, transaction classification, rail reconciliation, verified monthly snapshot or credit result is implied. Those later actions must preserve this source lineage and current authority.

Local evidence: 1,339 complete non-TIA PHP tests / 6,618 assertions at 100% application line coverage, zero PHPStan errors, and all three evidence-boundary probes caught with clean teardown. Tests cover binary PDF preservation, encrypted storage, plaintext exclusion from manifests/receipts, stale revisions, exact retry, failed extraction rollback, current-authority denial, malformed inputs, original integrity, immutable rows and migration rollback. The combined PostgreSQL suite includes same-Party concurrent imports and competing imports by different mandate holders. HTTP upload/download and Auditor verification remain in progress; this is not complete checkpoint 2 acceptance.
