...
**Indexes:** `audit_report_assignment_key` on (id, assignment_id) — unique; `audit_report_root_assignment` on (assignment_id) — unique; `audit_reports_amends_id_unique` on (amends_id) — unique; `audit_reports_pkey` on (id) — unique
