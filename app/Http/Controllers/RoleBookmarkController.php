...
class RoleBookmarkController extends Controller
{
    ...
    public function resume(Request $request, string $role, GetRoleBookmark $action): RedirectResponse
    {
        $bookmark = $action->handle((int) $request->user()?->getAuthIdentifier(), $role);

        return redirect()->route($bookmark['route'], array_merge($bookmark['parameters'], $bookmark['query']));
    }
}
